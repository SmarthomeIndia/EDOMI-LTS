...
?><? class class_email
{
    ...
    private function com_connect()
    {
        if ($this->Auth == 'ssl') {
            if (extension_loaded('openssl')) {
                ...
                writeToLog(-1, false, 'EMAIL-Fehler: SSL-Verbindung nicht möglich (openssl-Extension fehlt)');
                return false;
            }
        }
        if ($this->Socket = fsockopen($this->Host, $this->Port, $errno, $errstr, $this->timeout)) {
            if (substr($this->mailGetResponse("fsockopen"), 0, 3) != '220') {
                writeToLog(-1, false, 'EMAIL-Fehler: Verbindung vom Host abgelehnt.');
                fclose($this->Socket);
                return false;
            }
        } else {
            writeToLog(-1, false, 'EMAIL-Fehler: Socket konnte nicht geöffnet werden.');
            return false;
        }
        return true;
    }
        ...
        if ($this->Auth == 'tls') {
            if (extension_loaded('openssl')) {
                ...
                if (substr($this->mailGetResponse("STARTTLS"), 0, 3) != '220') {
                    writeToLog(-1, false, 'EMAIL-Fehler: TLS-Verbindung vom Host abgelehnt.');
                    $this->mailReset();
                    return false;
                }
                ...
                if (substr($this->mailGetResponse("SEND HELLO MESSAGE"), 0, 3) != '250') {
                    writeToLog(-1, false, 'EMAIL-Fehler: Verbindung vom Host abgelehnt. TLS gescheitert.');
                    $this->mailReset();
                    return false;
                }
                $this->mailSendRequest("AUTH LOGIN");
                if (substr($this->mailGetResponse("AUTH LOGIN"), 0, 3) != '334') {
                    writeToLog(-1, false, 'EMAIL-Fehler: Verbindung vom Host abgelehnt. Authorisierung gescheitert.');
                    $this->mailReset();
                    return false;
                }
                $this->mailSendRequest(base64_encode($this->Login));
                if (substr($this->mailGetResponse("AUTH Username"), 0, 3) != '334') {
                    writeToLog(-1, false, 'EMAIL-Fehler: Verbindung vom Host abgelehnt. Zugangsdaten fehlerhaft.');
                    $this->mailReset();
                    return false;
                }
                ...
                writeToLog(-1, false, 'EMAIL-Fehler: TLS-Verbindung nicht möglich (openssl-Extension fehlt)');
                $this->mailReset();
                return false;
            }
        }
    ...
    private function com_sendmail()
    {
        ...
        if (substr($response, 0, 3) != '250') {
            writeToLog(-1, false, 'EMAIL-Fehler: Email konnte nicht gesendet werden (Reply-Code: ' . substr($response, 0, 3) . ' / Message-ID=' . $mailId . ').');
            $this->mailDisconnect();
            return false;
        }
        $this->mailDisconnect();
