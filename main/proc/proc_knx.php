...
class procKnx
{
    ...
    public function __construct()
    {
        ...
        sql_connect();
        if (writeToLog(3, $this->cE_serverIpPort = $this->ipPortToRaw($this->cE_serverIp, $this->cE_serverPort), 'KNX-connection: Control-Endpoint: ' . $this->cE_serverIp . ':' . $this->cE_serverPort, 'FATALERROR') === false) {
            exitSelf();
        }
        if (writeToLog(3, $this->dE_serverIpPort = $this->ipPortToRaw($this->dE_serverIp, $this->dE_serverPort), 'KNX-connection: Data-Endpoint: ' . $this->dE_serverIp . ':' . $this->dE_serverPort, 'FATALERROR') === false) {
            exitSelf();
        }
            ...
            if ($this->cE_socket !== false && $this->dE_socket !== false) {
                if (socket_bind($this->cE_socket, 0, $this->cE_serverPort) && socket_bind($this->dE_socket, 0, $this->dE_serverPort)) {
                    ...
                    if (!(global_logTraceLevelKnx == 1 || global_logTraceLevelKnx == 3)) {
                        writeToLog(3, false, 'socket_bind not possible.');
                    }
                    $this->trace(true, 'socket_bind not possible.');
                }
            } else {
                if (!(global_logTraceLevelKnx == 1 || global_logTraceLevelKnx == 3)) {
                    writeToLog(3, false, 'socket_create not possible.');
                }
                $this->trace(true, 'socket_create not possible.');
            }
            socket_close($this->cE_socket);
            socket_close($this->dE_socket);
            if ($this->procControl != 1) {
                if (!(global_logTraceLevelKnx == 1 || global_logTraceLevelKnx == 3)) {
                    writeToLog(3, false, 'KNX-connection lost.');
                }
                $this->trace(true, 'KNX-connection lost.');
                $this->procData[7]++;
                sleep(3);
            }
            if ($this->procTrigger[0]) {
    ...
    private function CONTROL_CONNECT_RESPONSE($response)
    {
        ...
        if ($this->cE_channelId >= 0 && $err == 0) {
            ...
            $errMsg = '(unbekannt)';
            if ($err == 0x22) {
                $errMsg = 'E_CONNECTION_TYPE (connection type not supported)';
            }
            if ($err == 0x23) {
                $errMsg = 'E_CONNECTION_OPTION (connection options not supported)';
            }
            if ($err == 0x24) {
                $errMsg = 'NO_MORE_CONNECTIONS (maximum connection count reached)';
            }
    ...
    private function DATA_TUNNELING_REQUEST_RECEIVE($response)
    {
        ...
        if ($this->byte($response, 8) == $this->cE_channelId) {
            ...
            if (($this->byte($response, 20) & 0x03) || (($this->byte($response, 21) & 0xC0) == 0xC0)) {
                ...
                if ($receiveRtg == $this->dE_tunneling_request_seqCounter_receive) {
                    ...
                    if ($receiveMsgTyp == 0x2E) {
                        ...
                        if (($this->byte($response, 21) & 0xC0) == 0x00) {
                            if ($gaData = getGADataFromGA($receiveGA, 1)) {
                                ...
                                if (global_knxUnknownGA & 1) {
                                    $this->trace(true, 'DE < | TUNNELING_REQUEST:L_Data.ind / Typ: Read / ErrMsg: unknown GA / SeqCounter: ' . $receiveRtg . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / PA: ' . $receivePA . ' / GA: ' . $receiveGA . ' / Raw: ' . $this->bytesToHex($response));
                                    $this->procData[2]++;
                                }
                            }
                            $this->procData[5]++;
                        ...
                        if (($this->byte($response, 21) & 0xC0) == 0x40) {
                            if ($gaData = getGADataFromGA($receiveGA, 1)) {
                                ...
                                if ($gaValue !== false) {
                                    ...
                                    $this->trace(true, 'DE < | TUNNELING_REQUEST:L_Data.ind / Typ: Response / ErrMsg: invalid value / SeqCounter: ' . $receiveRtg . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / PA: ' . $receivePA . ' / GA: ' . $receiveGA . ' = ' . $gaValueRaw . ' / Raw: ' . $this->bytesToHex($response));
                                }
                            } else {
                                writeToMonLog(1, 1, 1, 0, $receivePA, $receiveGA, '', 'HEX_' . $this->bytesToHex($receiveRawData));
                                if (global_knxUnknownGA & 1) {
                                    $this->trace(true, 'DE < | TUNNELING_REQUEST:L_Data.ind / Typ: Response / ErrMsg: unknown GA / SeqCounter: ' . $receiveRtg . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / PA: ' . $receivePA . ' / GA: ' . $receiveGA . ' / Raw: ' . $this->bytesToHex($response));
                                    $this->procData[2]++;
                                }
                            }
                            $this->procData[5]++;
                        ...
                        if (($this->byte($response, 21) & 0xC0) == 0x80) {
                            if ($gaData = getGADataFromGA($receiveGA, 1)) {
                                ...
                                if ($gaValue !== false) {
                                    ...
                                    sql_call("UPDATE edomiLive.RAMko SET visuts='" . getTimestampVisu() . "' WHERE (id=" . $gaData['id'] . ")");
                                    writeToMonLog(1, -1, 1, $gaData['id'], $receivePA, $receiveGA, $gaData['name'], $gaValueRaw);
                                    $this->trace(true, 'DE < | TUNNELING_REQUEST:L_Data.ind / Typ: Write / ErrMsg: invalid value / SeqCounter: ' . $receiveRtg . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / PA: ' . $receivePA . ' / GA: ' . $receiveGA . ' = ' . $gaValueRaw . ' / Raw: ' . $this->bytesToHex($response));
                                }
                            } else {
                                writeToMonLog(1, 2, 1, 0, $receivePA, $receiveGA, '', 'HEX_' . $this->bytesToHex($receiveRawData));
                                if (global_knxUnknownGA & 1) {
                                    $this->trace(true, 'DE < | TUNNELING_REQUEST:L_Data.ind / Typ: Write / ErrMsg: unknown GA / SeqCounter: ' . $receiveRtg . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / PA: ' . $receivePA . ' / GA: ' . $receiveGA . ' / Raw: ' . $this->bytesToHex($response));
                                    $this->procData[2]++;
                                }
                            }
                            $this->procData[5]++;
                        ...
                        $this->trace(true, 'DE < | TUNNELING_REQUEST / ErrMsg: unknown MessageCode: ' . dechex($receiveMsgTyp) . 'h / Raw: ' . $this->bytesToHex($response));
                    }
                } else if ($receiveRtg == ($this->dE_tunneling_request_seqCounter_receive - 1)) {
                    $ACK = true;
                    $INC = false;
                    if (global_knxLogSeqErr) {
                        $this->trace(true, 'DE < | TUNNELING_REQUEST / Hinweis: SequenceCounter does not fit (ACK): current value=' . $receiveRtg . ', expected value=' . $this->dE_tunneling_request_seqCounter_receive . ' / Raw: ' . $this->bytesToHex($response));
                        $this->procData[6]++;
                    }
                } else {
                    $ACK = false;
            ...
            $this->trace(true, 'DE < | TUNNELING_REQUEST / ErrMsg: ChannelId does not fit: current value=' . $this->byte($response, 8) . ', expected value=' . $this->cE_channelId . ' / Raw: ' . $this->bytesToHex($response));
            $this->procData[6]++;
        }
        if ($ACK) {
    ...
    private function DATA_TUNNELING_ACK($response)
    {
        ...
        if ($this->byte($response, 8) == $this->cE_channelId && $err == 0x00) {
            ...
            if (!$this->dE_tunneling_request_repeat) {
                $this->trace(true, 'DE < | TUNNELING_ACK / ErrCode: ' . dechex($err) . 'h / ErrMsg: ' . $errMsg . ' / SeqCounter: ' . $this->byte($response, 9) . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / repeat / Raw: ' . $this->bytesToHex($response));
                $this->dE_tunneling_request_repeat = true;
            } else {
                $this->trace(true, 'DE < | TUNNELING_ACK / ErrCode: ' . dechex($err) . 'h / ErrMsg: ' . $errMsg . ' / SeqCounter: ' . $this->byte($response, 9) . ' (' . $this->dE_tunneling_request_seqCounter_receive . ') / drop / Raw: ' . $this->bytesToHex($response));
                $this->dE_tunneling_request_repeat = false;
                $this->dE_tunneling_ack = true;
            }
            $this->procData[6]++;
    ...
    {
        writeToLog(3, sql_disconnect(), 'Database: close connection');
        writeToLog(3, true, 'KNX process stopped');
        exit();
    }
} 
?>
