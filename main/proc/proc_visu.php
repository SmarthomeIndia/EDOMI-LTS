...
error_reporting(0);
writeToLog(7, true, 'Prozess VISU gestartet');
sql_connect();
$procControl = null;
...
do {
    ...
    if ($procTrigger[0]) {
        ...
        if ($procControl == 1) {
            writeToLog(7, true, 'Prozess VISU beenden...');
            writeToVisuLog(true, '', '', 'Beenden', 'sE');
        }
        resetProcData($procData);
        $procData[0] = 0;
        $procData[19] = 1;
    ...
    if ($procControl == 2) {
        writeToVisuLog(true, '', '', 'Gestartet', 'sS');
        ...
        if ($socket->socketOpen()) {
            ...
            do {
                ...
                if ($procTrigger[0]) {
                    ...
                    if ($procControl == 3) {
                        writeToLog(7, true, 'Prozess VISU pausieren und neustarten...');
                        writeToVisuLog(true, '', '', 'Beenden und neustarten', 'sE');
                    }
                }
        ...
        writeToVisuLog(true, '', '', 'Beendet', 'sX');
    } else {
        if ($procControl != 1) {
            usleep(100000);
        }
...
function exitSelf()
{
    ...
    writeToLog(7, sql_disconnect(), 'Datenbank: Verbindung schließen');
    writeToLog(7, true, 'Prozess VISU beendet');
    exit();
}
