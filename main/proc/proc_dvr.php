...
class procDvr
{
    ...
    {
        writeToLog(8, true, 'Prozess DVR gestartet');
        sql_connect();
    ...
    private function proc_check()
    {
        ...
        if ($this->procTrigger[0]) {
            ...
            if ($this->procControl == 1) {
                writeToLog(8, true, 'Prozess DVRCAM beenden...');
            } else {
                return true;
            }
        }
    ...
    private function mountCheck()
    {
        if (global_dvrMountcheck) {
            ...
            if ($tmp === false && $this->isMounted === true) {
                writeToLog(8, false, 'Prozess DVR: Zielpfad ist nicht gemounted (Mountcheck) - Aufzeichnung wurde angehalten!');
            }
            $this->isMounted = $tmp;
        } else {
            $this->isMounted = true;
    ...
    {
        writeToLog(8, sql_disconnect(), 'Datenbank: Verbindung schließen');
        writeToLog(8, true, 'Prozess DVR beendet');
        exit();
    }
} ?>
