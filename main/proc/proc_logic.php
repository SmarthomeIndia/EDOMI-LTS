...
class procLogic
{
    ...
    {
        writeToLog(4, true, 'Prozess LOGIC gestartet');
    ...
    private function proc_check()
    {
        ...
        if ($this->procTrigger[0]) {
            ...
            if ($this->procControl == 1) {
                writeToLog(4, true, 'Prozess LOGIC beenden...');
            } else {
                return true;
            }
        }
    ...
    {
        writeToLog(4, true, 'Logikengine: Wartet');
        $return = 0;
        do {
            $this->loop_begin();
            $this->procData[19] = 1;
    ...
    private function logicMode_init()
    {
        ...
        logicMonitor_mode(1);
        writeToLog(4, true, 'Logikengine: Initialisierung');
        $this->procData[19] = 3;
        procStatus_setData(4, $this->procData);
        ...
        if (sql_getCount('edomiLive.RAMlogicLink', 'init<>2') == 0) {
            writeToLog(4, true, 'LBS-Initialisierung: ' . $maxElements . ' von ' . $maxElements . ' LBS initialisiert');
            $this->procData[1] = $maxElements;
            $this->procData[11] = $maxElements;
        } else {
            writeToLog(4, true, 'LBS-Initialisierung: ' . $t . ' von ' . $maxElements . ' LBS initialisiert / nicht initialisierte LBS:');
            $this->procData[1] = $t;
            $this->procData[11] = $maxElements;
            ...
            while ($n = sql_result($ss1)) {
                ...
                if ($nn !== false) {
                    writeToLog(4, true, 'Logikseite ' . $nn['pageid'] . ': LBS-Instanz ' . $nn['id'] . ' (' . $nn['functionid'] . ')');
                }
            }
            sql_close($ss1);
    ...
    private function logicMode_run()
    {
        global $logic_globalStartExec, $logic_globalStartExecQueue;
        logicMonitor_mode(2);
        writeToLog(4, true, 'Logikengine: Gestartet');
        writeGA(2, 1);
        $return = 0;
        writeToLog(4, true, 'LBS-Initialisierung: LBS-EXEC-Scripte werden ggf. gestartet (Queue)');
    ...
    {
        writeToLog(4, sql_disconnect(), 'Datenbank: Verbindung schließen');
        writeToLog(4, true, 'Prozess LOGIC beendet');
        exit();
    }
} ?>
