...
error_reporting(0);
writeToLog(6, true, 'Prozess PHONE gestartet');
sql_connect();
$pingCounter = 0;
$mainTimerOldProc = 0;
$procControl = null;
...
do {
    ...
    if ($procTrigger[0]) {
        ...
        if ($procControl == 1) {
            writeToLog(6, true, 'Prozess PHONE beenden...');
        }
        $procData[0] = 0;
        $procData[1] = 0;
        $procData[2] = 0;
...
function exitSelf()
{
    ...
    writeToLog(6, sql_disconnect(), 'Datenbank: Verbindung schließen');
    writeToLog(6, true, 'Prozess PHONE beendet');
    exit();
}
