...
function restore($fn)
{
    if (file_exists(BACKUP_PATH . '/' . $fn)) {
        ...
        fwrite($fh, 'echo "Restore started"' . "\n");
        fwrite($fh, 'echo "--------------------------------------------------------------------------------"' . "\n");
        fwrite($fh, 'sleep 3s' . "\n");
        fwrite($fh, 'clear' . "\n");
        ...
        fwrite($fh, 'echo "--------------------------------------------------------------------------------"' . "\n");
        fwrite($fh, 'echo "Restore finished. Reboot in 5 seconds..."' . "\n");
        fwrite($fh, 'echo "--------------------------------------------------------------------------------"' . "\n");
        fwrite($fh, 'sleep 5s' . "\n");
        fwrite($fh, 'reboot' . "\n");
        fwrite($fh, 'exit' . "\n");
    ...
    if (file_exists(MAIN_PATH . '/www/data/tmp/' . $fn)) {
        if (preg_match('/\_(.*?)\./s', $fn, $v) > 0) {
            ...
            if ($ok) {
                ...
                fwrite($fh, 'fi' . "\n");
                fwrite($fh, 'echo "--------------------------------------------------------------------------------"' . "\n");
                fwrite($fh, 'echo "Update completed. Reboot in 5 seconds..."' . "\n");
                fwrite($fh, 'echo "--------------------------------------------------------------------------------"' . "\n");
                fwrite($fh, 'sleep 5s' . "\n");
                fwrite($fh, 'reboot' . "\n");
                fwrite($fh, 'exit' . "\n");
                ...
                deleteFiles(MAIN_PATH . '/www/data/tmp/' . $fn);
                writeToLog(0, false, 'EDOMI-Update  (' . $fn . ') failed: wrong version!');
                return false;
            }
        }
    }
...
function autoupdate($download)
{
    if (!isEmpty(global_urlAutoupdate)) {
        ...
        if ($download == 2) {
            writeToLog(0, true, 'EDOMI-Autoupdate: check update availability');
        }
...
{
    writeToLog(0, true, 'Logs/Archive/cleanup autobackups ');
...
function dbRotate_archivKo()
{
    ...
    while ($n = sql_result($ss1)) {
        ...
        if (sql_affectedRows() > 0) {
            ...
            writeToLog(0, true, 'data archive (' . $n['id'] . '): ' . ($cntBefore - $cntAfter) . ' old entries removed, ' . $cntAfter . ' entries leftover');
            if ($n['outgaid'] > 0) {
    ...
    while ($archiv = sql_result($ss1)) {
        ...
        if ($tmp !== false && $archiv['coffset'] >= 1) {
            ...
            if ($changed) {
                ...
                writeToLog(0, true, 'data archive (' . $archiv['id'] . ') consensed: ' . ($cntBefore - $cntAfter) . ' entries from ' . $cntBefore . ' calculated and removed, ' . $cntAfter . ' entries leftover');
                if ($archiv['outgaid'] > 0) {
...
function dbRotate_archivMsg()
{
    ...
    while ($n = sql_result($ss1)) {
        ...
        if (sql_affectedRows() > 0) {
            ...
            writeToLog(0, true, 'message archive (' . $n['id'] . '): ' . ($cntBefore - $cntAfter) . ' old entries removed, ' . $cntAfter . ' entries leftover');
            if ($n['outgaid'] > 0) {
                writeGA($n['outgaid'], $cntAfter);
            }
        }
...
function dbRotate_archivPhone()
{
    ...
    while ($n = sql_result($ss1)) {
        ...
        if (sql_affectedRows() > 0) {
            ...
            writeToLog(0, true, 'call archive (' . $n['id'] . '): ' . ($cntBefore - $cntAfter) . ' old entries removed, ' . $cntAfter . ' entries leftover');
            if ($n['outgaid'] > 0) {
                writeGA($n['outgaid'], $cntAfter);
            }
        }
...
function dbRotate_archivCam()
{
    ...
    while ($n = sql_result($ss1)) {
        ...
        if (sql_affectedRows() > 0) {
            ...
            writeToLog(0, true, 'camera archive (' . $n['id'] . '): ' . ($cntBefore - $cntAfter) . ' old pictures removed, ' . $cntAfter . ' pictures leftover');
            if ($n['outgaid'] > 0) {
                writeGA($n['outgaid'], $cntAfter);
            }
        }
    ...
    foreach ($files as $pathFn) {
        if (is_file($pathFn)) {
            ...
            if (!sql_result($ss1)) {
                writeToLog(0, true, 'camera archive: orphaned file ' . basename($pathFn) . ' removed');
                deleteFiles($pathFn);
            }
            sql_close($ss1);
        }
...
function saveProject($mode)
{
    ...
    if ($mode == 1 && $project !== false) {
        writeToTmpLog(0, 'archive working project: ' . ajaxValueHTML($project['name']));
    ...
    if ($mode == 2 && $project !== false) {
        writeToTmpLog(0, 'archive working project as duplicate: ' . ajaxValueHTML($project['name'] . ((global_duplicateSuffix) ? '-KOPIE' : '')));
        ...
        if ($tmp = sql_insertId()) {
            ...
            writeToTmpLog(1, 'Error: entry in database failed', true);
            $r[1]++;
        }
    }
    if ($mode == 3 && $project !== false) {
        writeToTmpLog(0, 'download working project: ' . ajaxValueHTML($project['name']));
        ...
        if (!isEmpty($tmp)) {
            ...
            writeToTmpLog(1, 'generated download-filename: ' . ajaxValueHTML($tmp));
            $r[2] = $tmp;
        } else {
            writeToTmpLog(1, 'Error: filename could not be generated', true);
            $r[1]++;
        }
    }
    createInfoFile(MAIN_PATH . '/www/data/tmp/projectsave_status.txt', array(0));
    if (!isEmpty($fn) && $project !== false) {
        writeToLog(0, true, 'project archive/download: ' . basename($fn));
        ...
        writeToTmpLog(1, 'no working project available or unknown error', true);
        $r[1]++;
    }
...
function loadProject($prjId)
{
    ...
    if (file_exists(MAIN_PATH . '/www/data/projectarchiv/' . $fn)) {
        ...
        writeToTmpLog(1, 'project: EDOMI-Version ' . $version);
        $version = floatVal($version);
        $tmp = edomi_update($version, false);
        if (!isEmpty($tmp)) {
            writeToTmpLog(1, 'project: apply updates (' . $tmp . ')');
            writeToLog(0, true, 'project ' . $fn . ': update EDOMI version ' . $tmp);
        }
        ...
        writeToTmpLog(1, 'import logic modules');
        $tmp = lbs_importAll(2);
        $errCount[0] += $tmp[1];
        createInfoFile(MAIN_PATH . '/www/data/tmp/projectload_status.txt', array(66));
        writeToTmpLog(1, 'import visu-elements');
        $tmp = vse_importAll(2);
        $errCount[0] += $tmp[1];
    } else {
        writeToTmpLog(1, 'file not found', true);
        $errCount[1]++;
    }
...
{
    $errCount = array(0, 0);
    writeToTmpLog(0, 'delete current working project and create empty working project');
    ...
    writeToTmpLog(1, 'initialize project database');
    if (init_DB_Project()) {
        writeToTmpLog(1, 'delete files');
        deleteFiles(MAIN_PATH . '/www/data/project/visu/img/*.*');
        deleteFiles(MAIN_PATH . '/www/data/project/visu/etc/*.*');
        deleteFiles(MAIN_PATH . '/www/data/project/*.*');
        writeToTmpLog(1, 'delete non-archived projects');
        sql_call("DELETE FROM edomiAdmin.project WHERE (savedate IS NULL)");
        writeToTmpLog(1, 'create new working project');
        sql_call("UPDATE edomiAdmin.project SET edit=0");
        ...
        writeToTmpLog(1, 'project database could not be initialized', true);
        $errCount[1]++;
    }
...
function importVse($mode)
{
    ...
    writeToTmpLog(0, 'import visu-elements');
    createInfoFile(MAIN_PATH . '/www/data/tmp/importvse_status.txt', array(0));
    if ($mode == -1) {
        $tmp = vse_importAll(1);
        $errCount[0] += $tmp[1];
        writeToTmpLog(1, $tmp[0] . ' successfully imported visu-elements / ' . $errCount[0] . ' broken elements');
    } else if ($mode == -2) {
        $c = 0;
        ...
        writeToTmpLog(1, $c . ' successfully imported visu-elements / ' . $errCount[0] . ' broken elements');
...
function importLbs($mode)
{
    ...
    writeToTmpLog(0, 'import logic-modules');
    createInfoFile(MAIN_PATH . '/www/data/tmp/importlbs_status.txt', array(0));
    if ($mode == -1) {
        $tmp = lbs_importAll(1);
        $errCount[0] += $tmp[1];
        writeToTmpLog(1, $tmp[0] . ' logic-modules successfully imported / ' . $errCount[0] . ' failed modules');
    } else if ($mode == -2) {
        $c = 0;
        ...
        writeToTmpLog(1, $c . ' logic-modules successfully imported / ' . $errCount[0] . ' failed modules');
