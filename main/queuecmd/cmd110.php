...
function phoneCallFromDB($phoneId, $delay)
{
    if (global_phoneGatewayActive) {
        if ($phoneId > 0) {
            ...
            if ($n = sql_result($ss1)) {
                writeToMonLog(2, 4, null, null, null, null, 'outgoing call', 'db.phoneBook.id=' . $phoneId . ' / Nr: (' . $n['phone1'] . ')' . $n['phone2']);
                if (!fritzbox_Call($n['phone1'] . $n['phone2'], $delay)) {
                    writeToLog(0, false, 'phone: call failed (db.phoneBook.id=' . $phoneId . ')');
                }
            }
            sql_close($ss1);
        } else {
            writeToMonLog(2, 4, null, null, null, null, 'hangup', '');
            if (!fritzbox_HangUp()) {
                writeToLog(0, false, 'phone: hangup failed');
            }
        }
    }
}
