...
require(MAIN_PATH."/main/include/php/incl_cmd.php"); require(MAIN_PATH."/main/include/php/incl_process.php");
require(MAIN_PATH."/www/admin/include/php/base.php"); require(MAIN_PATH."/www/admin/include/php/incl_items.php");
    ...
    public function import_items() { 
        foreach ($this->importData as $k1 => $v1) { 
            ...
            foreach ($v1 as $k2 => $v2) { 
                ...
                if ($k1=='#sequences') {$this->import_add_sequence($v2['id']);
                } 
                if ($k1=='#scenes') {$this->import_add_scene($v2['id']);
                } 
                if ($k1=='#timers') {$this->import_add_timer($v2['id']);
                }
                if ($k1=='#agendas') {$this->import_add_agenda($v2['id']);
                } 
                if ($k1=='#cameras') {$this->import_add_camera($v2['id']);
                } 
                if ($k1=='#ipCommands') {$this->import_add_ipCommand($v2['id']);
                } 
                if ($k1=='#irCommands') {$this->import_add_irCommand($v2['id']);
                } 
                if ($k1=='#emails') {$this->import_add_email($v2['id']);
                } 
                if ($k1=='#diagrams') {$this->import_add_diagram($v2['id']);
                } 
                if ($k1=='#presenceSimulations') {$this->import_add_presenceSimulation($v2['id']);
                } 
                if ($k1=='#remoteAccesses') {$this->import_add_remoteAccess($v2['id']);
                } 
                if ($k1=='#phoneNumbers') {$this->import_add_phoneNumber($v2['id']);
                } 
                if ($k1=='#phoneCallTriggers') {$this->import_add_phoneCallTrigger($v2['id']);
                } 
                if ($k1=='#visus') {$this->import_add_visu($v2['id']);
                } 
                if ($k1=='#visuAccounts') {$this->import_add_visuAccount($v2['id']);
                } 
                if ($k1=='#logicPages') {$this->import_add_logicPage($this->importData['#logicPages'],$v2['id']);
                } 
            } 
        } 
...
public function exportSave($fileName) { 
    foreach ($this->exportTree as $k => $v) { 
        if (is_null($v)) { unset($this->exportTree[$k]); 
        } 
    } 
...
private function export_add_folder($id) { 
    ...
    while ($folder=sql_result($ss2)) { 
        ...
        foreach ($this->dbMeta as $db => $void) { 
            ...
            while ($item=sql_result($ss1)) { 
                $emptyFolder=false; $this->export_add_item($db,$item['id'],true,true); 
            } 
            sql_close($ss1); 
        } 
        if ($emptyFolder) { 
        ...
        if (!$r) { 
            if ($id>0) { 
                ...
                if ($item!==false) { 
                    ...
                    $fn='export_add_item_'.$db; $this->$fn($tree,$r,$item,$this->export_is_inFolders($item['folderid']),$noLink); 
                } 
            } 
        } else if ($noLink) { 
...
{ 
    ...
    sql_close($ss1); $treeThis['includePage_id']=$this->export_add_item('editVisuPage',$tmp['includeid']); 
    
    if ($addGlobalInkludes || ($tmp['pagetyp']==0 && $tmp['globalinclude']==1)) { 
        $ss1=sql_call("SELECT id FROM edomiProject.editVisuPage WHERE (visuid=".$tmp['visuid']." AND pagetyp=2) ORDER BY id ASC"); while ($n=sql_result($ss1)) { 
            $this->export_add_item('editVisuPage',$n['id']); } sql_close($ss1); 
    } 
} 
...
private function export_parse_command($tmp) { 
    ...
    if ($tmp['outmode']==13) { $tmp['outid'] = $this->export_add_item('editArchivKo',$tmp['outid']); } 
    if ($tmp['outmode']==14) { $tmp['outid'] = $this->export_add_item('editArchivMsg',$tmp['outid']); } 
    if ($tmp['outmode']==2) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); } 
    if ($tmp['outmode']==3) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editKo',$tmp['outvalue']); } 
    if ($tmp['outmode']==4) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); } 
    if ($tmp['outmode']==5) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); } 
    if ($tmp['outmode']==6) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editKo',$tmp['outvalue']); } 
    if ($tmp['outmode']==7) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); } 
    if ($tmp['outmode']==8) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); } 
    if ($tmp['outmode']==9) { $tmp['outid'] = $this->export_add_item('editKo',$tmp['outid']); } 
    if ($tmp['outmode']==10) { $tmp['outid'] = $this->export_add_item('editScene',$tmp['outid']); } 
    if ($tmp['outmode']==11) { $tmp['outid'] = $this->export_add_item('editSequence',$tmp['outid']); } 
    if ($tmp['outmode']==40) { $tmp['outid'] = $this->export_add_item('editArchivKo',$tmp['outid']); } 
    if ($tmp['outmode']==41) { $tmp['outid'] = $this->export_add_item('editArchivMsg',$tmp['outid']); } 
    if ($tmp['outmode']==12) { $tmp['outid'] = $this->export_add_item('editArchivCam',$tmp['outid']); } 
    if ($tmp['outmode']==50) { $tmp['outid'] = $this->export_add_item('editArchivKo',$tmp['outid']); } 
    if ($tmp['outmode']==51) { $tmp['outid'] = $this->export_add_item('editArchivMsg',$tmp['outid']); } 
    if ($tmp['outmode']==52) { $tmp['outid'] = $this->export_add_item('editArchivCam',$tmp['outid']); } 
    if ($tmp['outmode']==53) { $tmp['outid'] = $this->export_add_item('editArchivPhone',$tmp['outid']); } 
    if ($tmp['outmode']==15) { $tmp['outid'] = $this->export_add_item('editIp',$tmp['outid']); } 
    if ($tmp['outmode']==16) { $tmp['outid'] = $this->export_add_item('editIr',$tmp['outid']); } 
    if ($tmp['outmode']==20) { $tmp['outid'] = $this->export_add_item('editEmail',$tmp['outid']); } 
    if ($tmp['outmode']==22) { $tmp['outid'] = $this->export_add_item('editPhoneBook',$tmp['outid']); } 
    if ($tmp['outmode']==21) { $tmp['outid'] = $this->export_add_item('editVisuPage',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editVisuUser',$tmp['outvalue']); } 
    if ($tmp['outmode']==28) { $tmp['outid'] = $this->export_add_item('editVisu',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editVisuUser',$tmp['outvalue']); } 
    if ($tmp['outmode']==24) { $tmp['outid'] = $this->export_add_item('editVisu',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editVisuSnd',$tmp['outvalue']); } 
    if ($tmp['outmode']==25) { $tmp['outid'] = $this->export_add_item('editVisuUser',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editVisuSnd',$tmp['outvalue']); } 
    if ($tmp['outmode']==26) { $tmp['outid'] = $this->export_add_item('editVisu',$tmp['outid']); } 
    if ($tmp['outmode']==27) { $tmp['outid'] = $this->export_add_item('editVisuUser',$tmp['outid']); } 
    if ($tmp['outmode']==23) { $tmp['outid'] = $this->export_add_item('editVisu',$tmp['outid']); $tmp['outvalue'] = $this->export_add_item('editVisuUser',$tmp['outvalue']); } 
    if ($tmp['outmode']==30) { } $r=array('type' => $this->commandTypes[$tmp['outmode']][0]);
    if (!isEmpty($this->commandTypes[$tmp['outmode']][1])) {$r+=array($this->commandTypes[$tmp['outmode']][1] => $tmp['outid']);} 
    if (!isEmpty($this->commandTypes[$tmp['outmode']][2])) {$r+=array($this->commandTypes[$tmp['outmode']][2] => $tmp['outvalue']);} 
    if (!isEmpty($tmp['delay'])) {$r+=array('wait' => $tmp['delay']);} 
    return $r; 
} 
...
private function export_jsonEncode($arr,$reduce) { $r=$this->export_jsonEncode_rekursive(array($arr),0,$reduce); if (!isEmpty($r)) { return $r; } else { return false; } } 

private function export_jsonEncode_rekursive($arr,$tabLevel,$reduce) { 
    $r=''; foreach ($arr as $k=>$v) { 
        if (is_array($v)) { 
            if (!is_numeric($k)) { 
            ...
            $v=str_replace(chr(92),chr(92).chr(92),$v); $v=str_replace(chr(9), '\t',$v); 
...
{ 
    if ($this->export_is_inItems($db,$id)) {return true;} 
    $tmp=sql_getValues($db,'folderid','id='.$id); 
    if ($tmp!==false) { 
        if ($this->export_is_inFolders($tmp['folderid'])) {return true;} 
    } 
    if ($db=='editVisuPage') { } return false; 
} 
