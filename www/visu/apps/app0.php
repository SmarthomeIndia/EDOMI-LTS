...
/* 
*/ 
?><? ?><? require("../../shared/php/config.php"); require(MAIN_PATH."/www/shared/php/base.php"); require(MAIN_PATH."/www/shared/php/incl_http.php"); require(MAIN_PATH."/www/visu/include/php/config.php"); require(MAIN_PATH."/www/visu/include/php/base.php"); sql_connect(); $visuCmd=checkVisuSid($json1['visuId'],$sid,true); if ($cmd=='login' || $visuCmd) { cmd($cmd,$visuCmd); } else { $ss1=sql_call("SELECT * FROM edomiLive.visu ORDER BY id ASC"); if ($n=sql_result($ss1)) { $html=""; do { $html.="<option value='".$n['id']."' ".(($json1['visuId']==$n['id'])?"selected":"").">".ajaxEncode($n['name'])." (".$n['id'].")</option>"; } while ($n=sql_result($ss1)); ?>
		jsLogin("<?echo $json1['visuId'];?>","<?echo $html;?>");
<? } else { ?>
		serverNotReady("<?echo $json1['visuId'];?>","");
<? } } function cmd($cmd,$visuCmd) { global $json1,$json2,$sid,$vid; if ($cmd=='login') { $sid=loginVisu($json2[0],$json2[1],$json2[2]); if ($sid!==false) { ?>
			sid="<?echo $sid;?>";
...
<? } else { ?>
			...
			shakeObj("loginform");
<? } } if ($cmd=='initVisu') { if ($vid!=global_version) { ?>
			self.location.reload();
<? return; } ?>
		document.getElementById("windowContainer").style.display="none";
<? if (getEdomiStatus()==3) { $ss1=sql_call("SELECT * FROM edomiLive.visu WHERE (id=".$json1['visuId'].")"); if ($visu=sql_result($ss1)) { $visuUser=sql_getValues('edomiLive.visuUser','*','id='.$visuCmd['targetid']); ?>
...
<? $cssIndiColor1=sql_getValue('edomiLive.visuFGcol','color',"id='".$visu['indicolor']."'"); if (isEmpty($cssIndiColor1)) {$cssIndiColor1='#80e000';} $cssIndiColor2=sql_getValue('edomiLive.visuFGcol','color',"id='".$visu['indicolor2']."'"); if (isEmpty($cssIndiColor2)) {$cssIndiColor2='inherit';} ?>
...
<? $ss2=sql_call("SELECT * FROM edomiLive.visuMeta WHERE (".(($visuUser['preload']==1)?'typ=28 OR ':'')."typ=150 OR typ=27) AND visuid=".$json1['visuId']); while ($meta=sql_result($ss2)) { if ($meta['typ']==28) { if ($n=sql_getValues('edomiLive.visuImg','*','id='.$meta['id'])) { $fn='img-'.$n['id'].'.'.$n['suffix']; ?>
							preload_imagesFile.push("../data/liveproject/visu/img/<?ajaxEcho($fn);?>?<?echo $n['ts'];?>");
<? } } else if ($meta['typ']==150) { if ($n=sql_getValues('edomiLive.visuFont','*','id='.$meta['id'])) { if ($n['fonttyp']==0) { ?>
								tmp_fonts+="@font-face {font-family:font<?echo $n['id'];?>; src:local(<?ajaxValue($n['fontname']);?>);}\n";
<? } else { $fn='font-'.$n['id'].'.ttf'; ?>
								tmp_fonts+="@font-face {font-family:font<?echo $n['id'];?>; font-style:<?if ($n['fontstyle']==0) {echo 'normal';} else {echo 'italic';}?>; font-weight:<?if ($n['fontweight']==0) {echo 'normal';} else {echo 'bold';}?>; src:url('../../data/liveproject/visu/etc/<?ajaxEcho($fn);?>?<?echo $n['ts'];?>') format('truetype');}\n";
<? } } } else if ($meta['typ']==27) { if ($n=sql_getValues('edomiLive.visuAnim','*','id='.$meta['id'])) { ?>
