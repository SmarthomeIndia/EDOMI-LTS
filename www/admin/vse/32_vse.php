...
VSE_VSEID=function(elementId,obj,meta,property,isPreview,koValue) {
	...
	var n="<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
		n+="<tr style='height:"+mheight+"px;'>";
			...
			n+="<td width='20%' align='center'>&gt;</td>";
		n+="</tr>";
		n+="<tr><td colspan='3' align='center' style='border-top:1px dotted; border-bottom:1px dotted;'>"+((isPreview)?"":"<span class='app2_pseudoElement'>TERMINSCHALTUHR</span>")+"</td></tr>";
			...
			n+="<tr valign='bottom'>";
				n+="<td>";
					...
					n+="<table cellpadding='2' cellspacing='0' width='100%' height='100%' border='0' style='max-height:200px;'>";
						n+="<tr height='20%'>";
							n+="<td colspan='3' align='left'>"+visuElement_formNewInput("<?echo $json1['elementId'];?>",1,100)+"</td>";
						n+="</tr>";
	
						n+="<tr height='20%'>";
							n+="<td align='left'>"+visuElement_formNewInput("<?echo $json1['elementId'];?>",5,10)+"</td>";
							n+="<td align='right' style='white-space:nowrap;'>";
								n+="<table cellpadding='0' cellspacing='0' width='100%' height='100%' border='0' style='table-layout:auto;'>";
									n+="<tr>";
										n+="<td>"+visuElement_formNewInput("<?echo $json1['elementId'];?>",3,2)+"</td>";
										n+="<td width='1' align='center'>&nbsp;:&nbsp;</td>";
										n+="<td>"+visuElement_formNewInput("<?echo $json1['elementId'];?>",4,2)+"</td>";
									n+="</tr>";
								n+="</table>";
							n+="</td>";
							n+="<td>"+visuElement_formNewCheckmulti("<?echo $json1['elementId'];?>",9,"<?if (isEmpty($tmp_auxKo)) {echo 'display:none;';}?>")+"</td>";
						n+="</tr>";
						
						n+="<tr height='20%'>";
							n+="<td align='left' style='white-space:nowrap;'>"+visuElement_formNewInput("<?echo $json1['elementId'];?>",7,10)+"</td>";
							n+="<td align='left' style='white-space:nowrap;'>"+visuElement_formNewSelect("<?echo $json1['elementId'];?>",8)+"</td>";
							n+="<td align='left' style='white-space:nowrap;'>"+visuElement_formNewInput("<?echo $json1['elementId'];?>",6,10)+"</td>";
						n+="</tr>";
	
						n+="<tr height='20%'>";
							n+="<td colspan='3' align='right'>"+visuElement_formNewSelect("<?echo $json1['elementId'];?>",2)+"</td>";
						n+="</tr>";
...
<?
		if ($fd[0]>0) {
?>
							n+="<td><div class='controlButton' onClick='visuElement_callPhp(\"tsuDelete\",{elementId:\"<?echo $json1['elementId'];?>\",agendaId:\"<?echo $json1['agendaId'];?>\",dbId:\"<?echo $fd[0];?>\"},null);'>Löschen</div></td>";
							n+="<td><div class='controlButton' onClick='VSE_VSEID_ShowInfoEdit(\"<?echo $json1['elementId'];?>\",true);'>Abbrechen</div></td>";
							n+="<td><div class='controlButton' onClick='visuElement_callPhp(\"tsuSave\",{elementId:\"<?echo $json1['elementId'];?>\",agendaId:\"<?echo $json1['agendaId'];?>\",dbId:\"<?echo $fd[0];?>\"},visuElement_formGetValues(\"<?echo $json1['elementId'];?>\"));'><b>Übernehmen</b></div></td>";
<?
		} else {
?>
							n+="<td>&nbsp;</td>";
							n+="<td><div class='controlButton' onClick='VSE_VSEID_ShowInfoEdit(\"<?echo $json1['elementId'];?>\",true);'>Abbrechen</div></td>";
							n+="<td><div class='controlButton' onClick='visuElement_callPhp(\"tsuSave\",{elementId:\"<?echo $json1['elementId'];?>\",agendaId:\"<?echo $json1['agendaId'];?>\",dbId:\"<?echo $fd[0];?>\"},visuElement_formGetValues(\"<?echo $json1['elementId'];?>\"));'><b>Erstellen</b></div></td>";
<?
		}
?>
						n+="</tr>";					
...
<?
	...
	if ($cmd=='tsuSave') {
		$ok=true;
		if (isEmpty($json2[3])) {$json2[3]=0;}
		if (isEmpty($json2[4])) {$json2[4]=0;}

		if (strtotime($json2[5])===false) {$ok=false;}	
		$json2[5]=date('Y-m-d',strtotime($json2[5]));

		if (!isEmpty($json2[6])) {
			if (strtotime($json2[6])===false) {$ok=false;}	
...
<?
		...
		if (abs($t2-$t1)>31) {$t2=$t1+31;}	//max. 31 Tage (Ressourcen sparen)
...
<?
		...
		while ($n=sql_result($ss1)) {
			...
			if ($n['step']>0) {
				$info1='alle '.$n['step'];
				if ($n['unit']==0) {$info1.=' Tage';}
				if ($n['unit']==1) {$info1.=' Wochen';}
				if ($n['unit']==2) {$info1.=' Monate';}
				if ($n['unit']==3) {$info1.=' Jahre';}
				if (!isEmpty($n['date2'])) {$info1.=' bis '.sql_getDate($n['date2']);}
				$info1.='<br>';
			}
...
?>
				n+="<td align='left'>";
					n+="<div style='word-break:break-all;'><?echo escapeString($n['name'],1);?></div>";
					n+="<div style='margin-top:3px; word-break:break-all;'>&gt; <?if (isEmpty($status)) {echo "-?-";} else {echo escapeString($status,1);}?></div>";
				n+="</td>";
				n+="<td align='right'>";
					n+="<div style='white-space:nowrap;'><?echo sql_getDate($n['date1']);?> / <?echo sprintf("%02d",$n['hour']);?>:<?echo sprintf("%02d",$n['minute']);?>&nbsp;Uhr<div>";
					n+="<div style='margin-top:3px; white-space:nowrap;'><?echo $info1;?></div>";
				...
				n+="</td>";
			n+="</tr>";
			n+="<tr><td colspan='2'><div style='width:100%; height:1px; border-bottom:1px solid; opacity:0.25;'></div></td></tr>";
<?
		}
?>
		n+="</table>";
...
function PHP_VSE_VSEID_printEvents($targetId,$var5,$var6,$d1) {
	...
	$tmp=sql_getDate($d1);
	if ($var6==1 && $tmp==date('d.m.Y',strtotime('now -2 day'))) {$tmp='Vorgestern';}
	else if ($var6==1 && $tmp==date('d.m.Y',strtotime('now -1 day'))) {$tmp='Gestern';}
	else if ($var6==1 && $tmp==date('d.m.Y')) {$tmp='Heute';}
	else if ($var6==1 && $tmp==date('d.m.Y',strtotime('now +1 day'))) {$tmp='Morgen';}
	else if ($var6==1 && $tmp==date('d.m.Y',strtotime('now +2 day'))) {$tmp='Übermorgen';}
    else {$tmp=$global_weekdays[date('N',strtotime($tmp))-1].', '.$tmp;}
...
?>
			n+="<tr valign='top' style='margin:0; padding:5px;'><td align='left' colspan='2'><?echo $tmp;?></td></tr>";
<?
		}
		...
		do {
			if ($var5>0 || strtotime('now')<=strtotime($d1.' '.$n['hour'].':'.$n['minute'].':59')) {
				$ok=true;
?>
				n+="<tr valign='top' style='<?echo (($var5==1 && (strtotime('now')>strtotime($d1.' '.$n['hour'].':'.$n['minute'].':59')))?'opacity:0.75;':'');?> margin:0; padding:5px;'>";
					n+="<td align='left' width='1' style='white-space:nowrap;'><?echo sprintf("%02d",$n['hour']);?>:<?echo sprintf("%02d",$n['minute']);?></td>";
					n+="<td align='left' style='word-break:break-all;'><?echo escapeString($n['name'],1);?></td>";
				n+="</tr>";
<?
			}
		} while ($n=sql_result($ss1));		
...
###[HELP]###
Das Visuelement "Terminschaltuhr" stellt eine <link>konfigurierte Terminschaltuhr***1000-101</link> in der Visualisierung dar und ermöglicht das Steuern der Terminschaltuhr, sowie das Hinzufügen und Bearbeiten von Terminen.
...
<ul>
	<li>Terminschaltuhr: Auswahl der <link>konfigurierten Terminschaltuhr***1000-101</link>, die angezeigt und bearbeitet werden soll</li>
	...
	<li>
		...
		<ul>
			...
			<li>Wichtig: Die Terminschaltuhr kann unabhängig davon stets über das entsprechende KO gesteuert werden, Termine können bereits in der Konfiguration der Terminschaltuhr definiert werden.</li>
		</ul>
	</li>
	...
	<li>
		Datumsanzeige: optional wird ein Termindatum in der Terminauflistung ggf. mit "vorgestern" bis "übermorgen" ersetzt (alle anderen Zeitpunkte werden in der Form "Wochentag, tt.mm.jjjj" angezeigt) 
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>Hinweis: Wir der Zeitpunkt eines Termin erreicht (z.B. 17:00:00 Uhr), wird der Termin unabhängig von den o.g. Einstellungen <i>mindestens</i> noch 1 Minute lang angezeigt (z.B. bis 17:00:59). Die genaue Anzeigedauer hängt von der Einstellung "Aktualisierung" ab (s.u.), bzw. vom Zeitpunkt einer manuellen Aktualisierung.</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>Hinweis: Der maximal anzeigbare Zeitraum beträgt 31 Tage. Wird dieser Zeitraum überschritten, wird automatisch der Zeitraum "Beginn + 31 Tage" angezeigt.</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			<li>Hinweis: Die Terminauflistung wird unabhängig von dieser Einstellung bei jedem Seitenaufruf und manuell durch das Anklicken der Titelleiste (s.u.) aktualisiert.</li>
		</ul>
	</li>
</ul>
...
<ul>
	<li>Bereits in der <link>Konfiguration der Terminschaltuhr***1000-101</link> können Termine fest vorgegeben werden - diese sind in der Visualisierung nicht veränderbar!</li>
...
<ul>
	<li>
		Titelleiste:
		<ul>
			<li>hier werden Pfeil-Schaltflächen zum Blättern durch die Termine (Scrollen), sowie der Name der <link>Terminschaltuhr***1000-101</link> angezeigt</li>
			<li>ein Klick auf den Namen der Terminschaltuhr aktualisiert den Inhalt der Terminauflistung, bzw. wechselt aus der Bearbeitungsansicht (s.u.) zurück zur Terminauflistung</li>
		</ul>
	</li>
...
<ul>
	<li>
		Titelleiste:
		<ul>
			<li>hier werden Pfeil-Schaltflächen zum Blättern durch die konfigurierten Termine (Scrollen), sowie der Name der <link>Terminschaltuhr***1000-101</link> angezeigt</li>
	...
	<li>
		...
		<ul>
			...
			<li>durch Anklicken eines Termins kann dieser bearbeitet oder gelöscht werden (siehe auch: <link>Konfiguration einer Terminschaltuhr***1000-101</link>)</li>
