...
<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
    n+="
    <tr>
        <td align='center'>"+((isPreview)?"":"<span class='app2_pseudoElement'>BILD-URL/WEBSEITE</span>")+"</td>
    </tr>
    ";
    n+="
</table>";
...
var n="<img id='e-"+elementId+"-urlcontent'
            src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4//8/AwAI/AL+5gz/qwAAAABJRU5ErkJggg==' draggable='false'
            style='"+tmp+" margin:0; padding:0; border:none; overflow:hidden; pointer-events:none;'>";
n+="
...
var n="
<iframe id='e-"+elementId+"-urlcontent' src='about:blank'
        style='position:absolute; left:0; top:0; width:100%; height:100%; margin:0; padding:0; border:none; overflow:hidden; background:transparent;'></iframe>";
} else {
var n="
<iframe id='e-"+elementId+"-urlcontent' src='about:blank'
        style='position:absolute; left:0; top:0; width:100%; height:100%; margin:0; padding:0; border:none; overflow:hidden; background:url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAAXNSR0IArs4c6QAAAChJREFUCB1jZGBgMAZiZODDhMwDsn2AeAuyIFgApAgmCBeACaIIgAQBzpEEiaAV3YQAAAAASUVORK5CYII=);'></iframe>";
}
n+="
<div id='e-"+elementId+"-reloadanim' class='reloadAnim'></div>";
}
...
<ul>
    ...
    <li>Caching verhindern: ist diese Option aktiviert, wird an die URL ein Zeitstempel in der Form "?&TS..." bzw. "&TS..." angehängt, um das Zwischenspeichern
        der Inhalte im Browser-Cache zu verhindern
    </li>
    ...
    <li>
        Webseiten-Hintergrund (nur bei URL-Typ "Webseite"): legt fest, welcher Hintergrund angezeigt werden soll bis die Webseite geladen wurde und diese
        Einstellung ggf. überschreibt
        <ul>
            <li>Streifenmuster: Als Hintergrund wird ein Streifenmuster angezeigt, bis die Webseite geladen wurde (ggf. wird dann der Hintergrund der Webseite
                verwendet).
            </li>
            <li>kein Hintergrund (transparent): Es wird kein Hintergrund angezeigt, bis die Webseite geladen wurde (ggf. wird dann der Hintergrund der Webseite
                verwendet).
            </li>
        </ul>
    </li>
    ...
    <li>
        ...
        <ul>
            ...
            <li>"alle x Sekunden/Minuten/Stunden": das Bild bzw. die Webseite wird in dem ausgewählten Intervall aktualisiert (und ggf. zusätzlich per KO,
                s.o.)
            </li>
        </ul>
    </li>
...
<b>Wichtig:</b>
<link>Konfigurierte Bilder***1000-28</link> stehen für dieses Visuelement nur dann (als lokale URL) zu Verfügung, wenn ein konfiguriertes Bild in einer Visualisierung tatsächlich genutzt wird: Ungenutzte Bild-Dateien werden bei der
<link>Projektaktivierung***103-13</link> nicht übertragen.
