...
VSE_VSEID=function(elementId,obj,meta,property,isPreview,koValue) {
	...
	var n="<table cellpadding='0' cellspacing='0' border='1' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
	if (obj.dataset.var7==0) {
		n+="<tr style='height:"+mheight+"px;'>";
			...
			n+="<td width='20%' align='center'>&gt;</td>";
		n+="</tr>";
		if (obj.dataset.var5>0) {n+="<tr style='height:"+obj.dataset.var6+"px;'><td colspan='3' style='border-top:1px dotted;'><div style='position:relative; left:0; bottom:0; width:20%; height:"+obj.dataset.var6+"px; background:"+((obj.dataset.var5==1)?visu_indiColor:"var(--bgc1)")+";'></div></td></tr>";}
		n+="<tr><td colspan='3' align='center' style='border-top:1px dotted;'>"+((isPreview)?meta.itemText:"<span class='app2_pseudoElement'>KAMERAARCHIV</span>")+"</td></tr>";

	} else {
		n+="<col width='20%'>";
		n+="<col width='60%'>";
		n+="<col width='20%'>";
		n+="<tr><td colspan='3' align='center' style='border-bottom:1px dotted;'>"+((isPreview)?meta.itemText:"<span class='app2_pseudoElement'>KAMERAARCHIV</span>")+"</td></tr>";
		if (obj.dataset.var5>0) {n+="<tr style='height:"+obj.dataset.var6+"px;'><td colspan='3' style='border-bottom:1px dotted;'><div style='position:relative; left:0; bottom:0; width:20%; height:"+obj.dataset.var6+"px; background:"+((obj.dataset.var5==1)?visu_indiColor:"var(--bgc1)")+";'></div></td></tr>";}
		n+="<tr style='height:"+mheight+"px;'>";
			n+="<td width='20%' align='center'>&lt;</td>";
			n+="<td width='60%' align='center'><div style='max-height:"+mheight+"px; overflow:hidden;'>"+property[0]+"</div></td>";
			n+="<td width='20%' align='center'>&gt;</td>";
...
<?
		...
		if ($n!==false) {
			...
			if ($json1['timeMode']==0) {$tmp=sql_getDateTime($n['datetime']);}
			else if ($json1['timeMode']==1) {$tmp=sql_getDateTime($n['datetime'])."<span style='opacity:0.75;'>.".sprintf("%06d",$n['ms'])."</span>";}
			else if ($json1['timeMode']==2) {$tmp=$global_weekdays[date('N',strtotime($n['datetime']))-1]." / ".sql_getDateTime($n['datetime']);}
			else if ($json1['timeMode']==3) {$tmp=$global_weekdays[date('N',strtotime($n['datetime']))-1]." / ".sql_getDateTime($n['datetime'])."<span style='opacity:0.75;'>.".sprintf("%06d",$n['ms'])."</span>";}

			if (isEmpty($json1['camViewId'])) {$json1['camViewId']=0;}
...
VSE_VSEID_CONSTRUCT=function(elementId,obj) {
	...
	n+="<div id='e-"+elementId+"-viewport' style='display:block; position:absolute; left:0; top:0; width:100%; height:100%;'>";
		...
		if (obj.dataset.var7==0) {
			n+="<tr style='height:"+mheight+"px;'>";
				n+="<td width='20%' id='e-"+elementId+"-last'>&lt;</td>";
				n+="<td width='60%' id='e-"+elementId+"-info'><div id='e-"+elementId+"-infotext' style='max-height:"+mheight+"px; overflow:hidden;'></div></td>";
				n+="<td width='20%' id='e-"+elementId+"-next'>&gt;</td>";
			n+="</tr>";
			if (obj.dataset.var5>0) {n+="<tr style='height:"+obj.dataset.var6+"px;'><td colspan='3' style='border-top:1px solid;'><div id='e-"+elementId+"-pos' style='position:relative; left:0; bottom:0; width:100%; height:"+obj.dataset.var6+"px; background:"+((obj.dataset.var5==1)?visu_indiColor:"var(--bgc1)")+";'></div></td></tr>";}
			n+="<tr><td id='e-"+elementId+"-cmd' colspan='3' style='border-top:1px solid;'><span id='e-"+elementId+"-text'></span></td></tr>";
		
		} else {
			n+="<col width='20%'>";
			n+="<col width='60%'>";
			n+="<col width='20%'>";
			n+="<tr><td id='e-"+elementId+"-cmd' colspan='3' style='border-bottom:1px solid;'><span id='e-"+elementId+"-text'></span></td></tr>";
			if (obj.dataset.var5>0) {n+="<tr style='height:"+obj.dataset.var6+"px;'><td colspan='3' style='border-bottom:1px solid;'><div id='e-"+elementId+"-pos' style='position:relative; left:0; bottom:0; width:100%; height:"+obj.dataset.var6+"px; background:"+((obj.dataset.var5==1)?visu_indiColor:"var(--bgc1)")+";'></div></td></tr>";}
			n+="<tr style='height:"+mheight+"px;'>";
				n+="<td width='20%' id='e-"+elementId+"-last'>&lt;</td>";
				n+="<td width='60%' id='e-"+elementId+"-info'><div id='e-"+elementId+"-infotext' style='max-height:"+mheight+"px; overflow:hidden;'></div></td>";
				n+="<td width='20%' id='e-"+elementId+"-next'>&gt;</td>";
...
###[HELP]###
Das Visuelement "Kameraarchiv" ermöglicht das Anzeigen der Kamerabilder eines <link>konfigurierten Kameraarchivs***1000-82</link>.
...
<ul>
	<li>Kameraarchiv: Auswahl des <link>konfigurierten Kameraarchivs***1000-82</link>, dessen Inhalt angezeigt werden soll</li>

	<li>
		Kameraansicht: Auswahl einer <link>konfigurierten Kameraansicht***1000-83</link>, die für die Anzeige der Archivbilder verwendet werden soll
		<ul>
			<li>Hinweis: diese Angabe ist optional, ohne Angabe einer Kameraansicht wird das Original-Archivbild angezeigt</li>
			<li>Achtung: prinzipiell könnte hier auch eine Kameraansicht ausgewählt werden, die <i>nicht</i> zur konfigurierten Kamera des Archivs gehört (nicht zu empfehlen)</li>
		</ul>
	</li>
	...
	<li>
		Positionsanzeige: ggf. kann die (zeitliche) Position des aktuell angezeigten Archivbildes bezogen auf das gesamte Archiv angezeigt werden (unterhalb der Trennline der Titelzeile)
		<ul>
			<li>deaktiviert: keine Positionsanzeige</li>
			<li>Indikatorfarbe: die Position wird mit der <link>Indikatorfarbe***1000-21</link> dargestellt</li>
			<li>Zusatzhintergrundfarbe 1: die Position wird mit der <link>Zusatzhintergrundfarbe 1***1003</link> dargestellt</li>
		</ul>
	</li>
...
<ul>
	<li>
		Titelleiste:
		<ul>
			<li>hier werden Pfeil-Schaltflächen zum Blättern durch die Archivbilder, der Name des <link>Kameraarchivs***1000-82</link> und ein Zeitstempel des angezeigten Archivbilds angezeigt</li>
