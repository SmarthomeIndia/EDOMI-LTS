...
$tmp=sql_getValues('edomiProject.editArchivKo','outgaid','id='.$item['var1'].' AND outgaid>0');
if ($tmp!==false) {
...
###[EDITOR.PHP]###
<?
$property[0]=sql_getValue('edomiProject.editArchivKo','name','id='.$item['var1']);
?>
###[/EDITOR.PHP]###
...
VSE_VSEID=function(elementId,obj,meta,property,isPreview,koValue) {
	...
	var n="<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
		n+="<tr style='height:"+mheight+"px;'>";
			...
			n+="<td width='20%' align='center'>&gt;</td>";
		n+="</tr>";
		n+="<tr><td colspan='3' align='center' style='border-top:1px dotted;'>"+((isPreview)?"":"<span class='app2_pseudoElement'>DATENARCHIV</span>")+"</td></tr>";
	n+="</table>";
	obj.innerHTML=n;
...
<?
		$ss1=sql_call("SELECT * FROM edomiLive.archivKoData WHERE (targetid=".$json1['archivId'].") ORDER BY datetime DESC,ms DESC LIMIT 0,".$json1['cursor']);
		$count=0;
...
<?
			if ($json1['timeMode']==1) {
?>
				n+="<td align='left' width='1' style='opacity:<?echo $json1['timeOpacity']/100;?>; white-space:nowrap;'><?echo sql_getTime($n['datetime']);?></td>";
<?
			} else if ($json1['timeMode']==2) {
?>
				n+="<td align='left' width='1' style='opacity:<?echo $json1['timeOpacity']/100;?>; white-space:nowrap;'><?echo sql_getDateTime($n['datetime']);?></td>";
<?
			} else if ($json1['timeMode']==3) {
?>
				n+="<td align='left' width='1' style='opacity:<?echo $json1['timeOpacity']/100;?>; white-space:nowrap;'><?echo sql_getDateTime($n['datetime']);?><span style='opacity:0.5;'>.<?echo sprintf("%06d",$n['ms']);?></span></td>";
<?
			}
?>
				n+="<td align='left' style='word-break:break-all;'><?echo escapeString($n['gavalue'],1);?>&nbsp;</td>";
...
?>
		n+="</table>";
		VSE_VSEID_callbackLoaded(<?echo $json1['elementId'];?>,n,<?echo $json1['cursor'];?>,<?echo (($scrollToTop)?'true':'false');?>,"<?echo escapeString(sql_getValue('edomiLive.archivKo','name','id='.$json1['archivId']),1);?>");
...
VSE_VSEID_ShowInfo=function(elementId,mode) {
	...
	if (d) {
		if (d.dataset.blocked==0) {	
			...
			visuElement_callPhp("archivInfo",{elementId:elementId,cursor:d.dataset.cursor,archivId:d.dataset.var1,load:d.dataset.var2,timeMode:d.dataset.var4,timeOpacity:d.dataset.var9},null);
		}
	}
}
...
###[HELP]###
Das Visuelement "Datenarchiv" stellt den Inhalt eines <link>konfigurierten Datenarchivs***1000-50</link> in der Visualisierung tabellarisch dar.
...
<ul>
	<li>Datenarchiv: Auswahl des <link>konfigurierten Datenarchivs***1000-50</link>, dessen Inhalt angezeigt werden soll</li>

	<li>
		Aktualisierung per KO: legt fest, ob die angezeigten Inhalte des Datenarchivs bei Änderung des KO1-Wertes (s.u.) aktualisiert werden soll
		<ul>
	...
	<li>
		...
		<ul>
			<li>die Daten werden blockweise aus dem Datenarchiv übertragen</li>
...
<b>Hinweis:</b>
Falls KO1 nicht angegeben wurde, wird das KO1 bei einer Aktivierung automatisch das Status-KO des Datenarchivs verknüpft (sofern vorhanden). Bei der Verwendung des Status-KO des Datenarchivs wird das Visuelement bei jeder Änderung des Archivinhaltes ggf. automatisch aktualisiert.
...
<ul>
	<li>
		Titelleiste:
		<ul>
			<li>hier werden Pfeil-Schaltflächen zum Blättern durch die Einträge (Scrollen), sowie der Name des <link>Datenarchivs***1000-50</link> angezeigt</li>
			<li>ein Klick auf den Namen des Datenarchivs aktualisiert den Inhalt des Visuelements</li>
		</ul>
	</li>
