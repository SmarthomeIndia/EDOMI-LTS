...
VSE_VSEID=function(elementId,obj,meta,property,isPreview,koValue) {
	...
	var n="<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
		n+="<tr style='height:"+mheight+"px;'>";
			...
			n+="<td width='20%' align='center'>&gt;</td>";
		n+="</tr>";
		n+="<tr><td colspan='3' align='center' style='border-top:1px dotted;'>"+((isPreview)?"":"<span class='app2_pseudoElement'>MELDUNGSARCHIV</span>")+"</td></tr>";
	n+="</table>";
	obj.innerHTML=n;
...
?>
			var n="";
			n+="<table cellpadding='2' cellspacing='0' width='100%' border='0' style='table-layout:auto;'>";
<?
...
?>
				n+="<tr valign='top' style='<?echo $parsed[1];?> <?echo $parsed[2];?>'>";
				n+="<td align='left' width='1' style='white-space:nowrap;'>";
<?
				if ($json1['timeMode']==1) {
?>
...
?>
					n+="<span style='opacity:<?echo $json1['timeOpacity']/100;?>;'><?echo sql_getDateTime($n['datetime']);?>.<?echo sprintf("%06d",$n['ms']);?></span>";
<?
				} else {
					if ($json1['formatIconSize']==0) {$json1['formatIconSize']=30;}	//"Auto" ohne Timestamp ist nicht möglich (CSS) => 30px
				}
				if (!isEmpty($parsed[3])) {
?>
					n+="<div <?if ($json1['timeMode']>=1) {echo "style='margin-top:5px;'";}?>><img src='../data/liveproject/visu/img/<?echo $parsed[3];?>' width='<?echo (($json1['formatIconSize']==0)?'100%':$json1['formatIconSize']);?>' height='auto' valign='middle' style='margin:0;' draggable='false'></img></div>";
<?
				}
?>
				n+="</td>";

				n+="<td align='left' style='word-break:break-all;'><?echo escapeString($parsed[0],1);?>&nbsp;</td>";
<?
				if ($json1['rowMode']==1) {
?>
					n+="<tr><td colspan='2'><div style='width:100%; height:1px;'></div></td></tr>";
<?
...
?>
				n+="<tr><td id='e-<?echo $json1['elementId'];?>-loadmore' colspan='2' align='center'><div style='margin:5px; padding:5px;'>&middot;&middot;&middot;</div></td></tr>";
				n+="<tr><td colspan='2'><div style='width:100%; height:1px; border-bottom:1px solid; opacity:0.25;'></div></td></tr>";
<?
			}
?>
...
<?
			...
			if ($n=sql_result($ss1)) {
				$parsed=PHP_VSE_VSEID_parseMsg($json1['formatMode'],$n['msg'],$n['formatid']);
?>
				n+="<table cellpadding='2' cellspacing='0' width='100%' border='0' style='table-layout:auto; overflow:hidden; text-align:left; margin:0; padding:2px; <?echo $parsed[2];?>'>";
					n+="<tr valign='top'>";
						n+="<td width='1'>";
<?
				if ($json1['timeMode']==1) {
?>
							n+="<div style='white-space:nowrap; pointer-events:none; margin-bottom:5px; <?echo $parsed[1];?>; opacity:<?echo $json1['timeOpacity']/100;?>;'><?echo sql_getTime($n['datetime']);?></span></div>";
<?
				} else if ($json1['timeMode']==2) {
?>
							n+="<div style='white-space:nowrap; pointer-events:none; margin-bottom:5px; <?echo $parsed[1];?>; opacity:<?echo $json1['timeOpacity']/100;?>;'><?echo sql_getDateTime($n['datetime']);?></span></div>";
<?
				} else if ($json1['timeMode']==3) {
?>
							n+="<div style='white-space:nowrap; pointer-events:none; margin-bottom:5px; <?echo $parsed[1];?>; opacity:<?echo $json1['timeOpacity']/100;?>;'><?echo sql_getDateTime($n['datetime']);?>.<?echo sprintf("%06d",$n['ms']);?></span></div>";
<?
				}
?>
							n+="<div id='e-<?echo $json1['elementId'];?>-delmsg' style='display:inline-block; margin-bottom:5px; padding:5px; border:1px solid; border-radius:3px; width:100%; box-sizing:border-box; text-align:center; white-space:nowrap; <?echo $parsed[1];?>'>Meldung löschen</div>";
<?
				if (!isEmpty($parsed[3])) {
?>
							n+="<div><img src='../data/liveproject/visu/img/<?echo $parsed[3];?>' width='<?echo (($json1['formatIconSize']==0)?'100%':$json1['formatIconSize']);?>' height='auto' valign='middle' style='margin:0;' draggable='false'></img></div>";
<?
				}
?>
						n+="</td>";
						n+="<td>";
							n+="<div style='word-break:break-all; pointer-events:none; <?echo $parsed[1];?>'><?echo escapeString($parsed[0],1);?>&nbsp;</div>";
						n+="</td>";
					n+="</tr>";
				n+="</table>";
...
function PHP_VSE_VSEID_parseMsg($mode,$msg,$formatid) {
	...
	if ($mode==1) {
		if ($formatid>0) {
			...
			if ($n!==false) {
				if ($n['fgid']>0) {$fg=sql_getValue('edomiLive.visuFGcol','color','id='.$n['fgid']);}
				if ($n['bgid']>0) {$bg=sql_getValue('edomiLive.visuBGcol','color','id='.$n['bgid']);}
				if ($n['imgid']>0) {
					if ($nn=sql_getValues('edomiLive.visuImg','*','id='.$n['imgid'])) {$img='img-'.$nn['id'].'.'.$nn['suffix'];}
				}
			}
			if (!isEmpty($fg)) {$fg='color:'.$fg.';';}
			if (!isEmpty($bg)) {$bg='background:'.$bg.';';}
		}
	}
...
VSE_VSEID_CONSTRUCT=function(elementId,obj) {
	...
	var n="<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
		...
		n+="</tr>";
		n+="<tr><td colspan='3' align='center' style='border-top:1px solid;'><div style='position:relative; height:100%;'><div id='e-"+elementId+"-edit' style='position:absolute; top:0; left:0; right:0; bottom:0; overflow-x:hidden; overflow-y:auto;'></div></div></td></tr>";
	n+="</table>";
	obj.innerHTML=n;
...
###[HELP]###
Das Visuelement "Meldungsarchiv" stellt den Inhalt eines <link>konfigurierten Meldungsarchivs***1000-60</link> in der Visualisierung tabellarisch dar.
...
<ul>
	<li>Meldungsarchiv: Auswahl des <link>konfigurierten Meldungsarchivs***1000-60</link>, dessen Inhalt angezeigt werden soll</li>
	...
	<li>
		...
		<ul>
			...
			<li>Einzelmeldung: Es wird nur ein Eintrag angezeigt (beginnend mit dem neuesten Eintrag). Die angezeigte Meldung kann mit einem Klick auf die Schaltfläche "Meldung löschen" dauerhaft aus dem Archiv entfernt werden.</li>
		</ul>
	</li>
	...
	<li>
		Formatierung: legt fest, ob und wie <link>Formatierungen***1000-155</link> angewendet werden
		<ul>
			<li>Formatierungen anzeigen: ist diese Option deaktiviert, werden sämtliche Formatierungen ignoriert</li>
			<li>Symbol Breite: legt die Breite des Symbols in Pixeln fest, die Höhe wird automatisch angepasst (0/leer: die Symbolbreite wird an den Zeitstempel angepasst)</li>
		</ul>
	</li>
</ul>
...
<ul>
	<li>
		Titelleiste:
		<ul>
			<li>hier werden Pfeil-Schaltflächen zum Blättern durch die Einträge (Scrollen), sowie der Name des <link>Meldungsarchivs***1000-60</link> angezeigt</li>
	...
	<li>
		Archiveinträge:
		<ul>
			<li>hier werden die geladenen Einträge des Archivs angezeigt (absteigend nach Zeitstempel sortiert, d.h. die neuesten Einträge werden am Anfag der Auflistung angezeigt)</li>
			<li>In der Darstellungsart "Einzelmeldung" wird nur 1 Eintrag angezeigt, dieser Eintrag kann durch einen Klick auf die Schaltfläche "Meldung löschen" dauerhaft aus dem Archiv entfernt werden.</li>
