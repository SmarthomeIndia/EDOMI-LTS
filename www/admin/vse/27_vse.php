...
if ($tmp !== false) {
    ...
    if (isEmpty($item['var3'])) {
        sql_call("UPDATE edomiLive.visuElement SET var3='" . sql_encodeValue($tmp['vmin']) . "' WHERE (id=" . $item['id'] . ")");
    }
    if (isEmpty($item['var4'])) {
        sql_call("UPDATE edomiLive.visuElement SET var4='" . sql_encodeValue($tmp['vmax']) . "' WHERE (id=" . $item['id'] . ")");
    }
}
?>
###[/ACTIVATION.PHP]###
...
<ul>
        ...
        <ul>
            <li>Linear: es wird eine lineare Darstellung erzeugt
                <ul>
                    ...
                    <li>Hinweis: die Ausrichtung der Grafik (horizontal oder vertikal) erfolgt stets automatisch anhand des Seitenverhältnisses (Breite:Höhe):
                        Ist "Breite &ge; Höhe" wird die Grafik horizontal ausgerichtet, ansonsten vertikal.
                    </li>
                </ul>
            </li>
    ...
    <li>
        ...
        <ul>
            <li>normal: die Anzeige erfolgt bei polarer Wertanzeige im Uhrzeigersinn (der Minimum-Wert entspricht dem Startwinkel), bei linearer Wertanzeige von
                links nach rechts (bzw. von oben nach unten)
            </li>
            <li>invertiert: die Anzeige erfolgt bei polarer Wertanzeige gegen den Uhrzeigersinn (der Minimum-Wert entspricht dem Endwinkel), bei linearer
                Wertanzeige von rechts nach links (bzw. von unten nach oben)
            </li>
        </ul>
    </li>
    ...
    <li>
        ...
        <ul>
            <li>beim Typ "Zeiger": diese Angabe bestimmt die Linienstärke der Zeiger (in Pixeln): die Angabe von "0" führt zu einer Skalierung in Abhängigkeit
                von der Größe des Visuelements
            </li>
            <li>beim Typ "Strich": diese Angabe bestimmt die Linienstärke des Strichs (in Pixeln), die radiale Länge entspricht stets der Angabe
                "Bereichsanzeige: Stärke" (auch wenn die Bereichsanzeige ausgeblendet ist)
            </li>
    ...
    <li>
        ...
        <ul>
            ...
            <li>
                ...
                <ul>
                    <li>Hinweis: der Typ "Segment" ist nur für die polare Wertanzeige verfügbar, bei einer linearen Wertanzeige wird "Segment" wie "Kontur
                        (runde Enden)" dargestellt.
                    </li>
                </ul>
            </li>
            <li>
                Stärke: bestimmt die Stärke der Bereichsanzeige in Pixeln (die Angabe von "0" führt zu einer automatischen Skalierung in Abhängigkeit von der
                Größe des Visuelements)
                <ul>
                    <li>beim Typ "Strich": diese Angabe bestimmt zudem die Länge des Strichs (in Pixeln)</li>
                    <li>beim Typ "Kreis": diese Angabe bestimmt zudem den Durchmesser(!) des Kreises (in Pixeln), sofern die Angabe "Zeiger: Stärke/Größe"=0 ist
                        (automatisch)
                    </li>
                    <li>beim Typ "Kontur/Balken": diese Angabe bestimmt zudem die Stärke der Kontur bzw. des Balkens (in Pixeln), sofern die Angabe "Zeiger:
                        Stärke/Größe"=0 ist (automatisch)
                    </li>
                </ul>
            </li>
            <li>
    ...
    <li>
        ...
        <ul>
            ...
            <li>Hinweis: Wenn der Minimum-Wert <i>größer</i> als der Maxmium-Wert ist, werden die Werte intern korrigiert (für eine invertierte Darstellung ist
                die Option "Richtung" entsprechend auszuwählen).
            </li>
        </ul>
    </li>
</ul>
    ...
    <li>
        ...
        <ul>
            ...
            <li>
                ...
                <ul>
                    <li>enthält der KO-Wert mehrere (durch ein Semikolon separierte) Werte, werden diese Werte als "Wert", "Minimum" und "Maximum"
                        interpretiert
                    </li>
