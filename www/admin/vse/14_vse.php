...
<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
    ...
    n+="
    <tr>
        <td colspan='2' style='border-top:1px dotted;'>"+((isPreview)?"":"<span class='app2_pseudoElement'>LISTE/TABELLE</span>")+"</td>
    </tr>
    ";
    n+="
</table>";
...
<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
    n+="
    <tr id='e-"+elementId+"-scroll' style='height:"+mheight+"px; display:none;'>";
        n+="
        <td width='50%' align='center' id='e-"+elementId+"-last'>&lt;</td>
        ";
        n+="
        <td width='50%' align='center' id='e-"+elementId+"-next'>&gt;</td>
        ";
        n+="
    </tr>
    ";
...
var b2="";
var n="
<table cellpadding='"+d.dataset.var5+"' cellspacing='0' border='0'
       style='width:100%;"+((d.dataset.var4==0)?" height:100%;":"")+((d.dataset.var9==1)?" table-layout:auto;":"")+"'>";
    ...
    if (d.dataset.var7==2) {nn+="
    <tr height='1'>";} else {nn+="
    <tr
    "+((d.dataset.var4>0)?" height='"+d.dataset.var4+"'":"")+">";}

    var tmp="";
    var isSet=false;
    for (var tt=colsMin;tt
    <colsMax
    ;tt++) {
    if (col0[tt]) {
    ...
    nn+="
    <td style='"+b1+b2+" color:var(--fgc2); background:var(--bgc2);'>"+(((col0[tt]=="" || col0[tt]===undefined))?"&nbsp;":col0[tt])+"</td>
    ";
    }
    if (isSet && d.dataset.var9==0) {n+="
    <colgroup>"+tmp+"</colgroup>
    ";}
    n+=nn;
    }
    ...
    n+="
    <tr
    "+((d.dataset.var4>0)?" height='"+d.dataset.var4+"'":"")+b0+" id='e-"+elementId+"-"+t+"'>";
    } else {
    n+="
    <tr
    "+((d.dataset.var4>0)?" height='"+d.dataset.var4+"'":"")+b0+">";
    }
    ...
    n+="
    <td style='"+b1+b2+"' id='e-"+elementId+"-"+t+"x"+tt+"'>"+((veVar.data[t][tt]=="")?"&nbsp;":veVar.data[t][tt])+"</td>
    ";
    } else {
    n+="
    <td style='"+b1+b2+"'>"+((veVar.data[t][tt]=="")?"&nbsp;":veVar.data[t][tt])+"</td>
    ";
    }
    } else {
    n+="
...
<ul>
    ...
    <li>
        ...
        <ul>
            <li>falls die Zeichenkette im Inhalt der 1. Spalte (bzw. der Zeile, falls keine Spalten erzeugt werden) vorhanden ist, wird die gesamte Zeile mit
                der
                <link>
                Zusatzhintergrundfarbe 1***1003</link> hinterlegt und die Zeichenkette aus dem Inhalt der 1. Spalte entfernt
            </li>
    ...
    <li>
        ...
        <ul>
            ...
            <li>1. Zeile ist Titelzeile: die vollstände 1. Zeile (ggf. einschließlich Spalten) wird als Titelzeile angezeigt und von einer Sortierung
                ausgeschlossen
            </li>
            <li>1. Zeile ist Titelzeile (Zeilenhöhe minimieren): wie zuvor, jedoch wird die Zeilenhöhe für die Titelzeile unabhängig von den o.g. Einstellungen
                so gering wie möglich eingestellt
            </li>
            <li>
                Hinweis: Die Titelzeile kann (optional) ausserdem zur Definition der jeweiligen Spaltenbreite (Tabellen) verwendet werden:
                <ul>
                    <li>die Breite jeder einzelnen Spalte kann als Prozentwert wie folgt definiert werden (als Spaltentrenner wird in diesem Beispiel das Komma
                        verwendet):
                    </li>
                    <li>"Titel1***10,Titel2***20,Titel3***70" führt zu Spaltenbreiten von 10%, 20% und 70%</li>
                </ul>
            </li>
            <li>Hinweis: Die Titelzeile wird ggf. in der
                <link>
                Zusatzvordergrundfarbe 2***1003</link> angezeigt und mit der
                <link>
                Zusatzhintergrundfarbe 2***1003</link> hinterlegt.
            </li>

            <li>
                Trennlinien: legt fest, ob die Titelzeile bzw. die einzelen Spalten mit einer Linie visuell abgetrennt werden sollen
                <ul>
                    <li>die Trennlinien der Titelzeile werden ggf. in der
                        <link>
                        Zusatzvordergrundfarbe 1***1003</link> angezeigt
                    </li>
                </ul>
            </li>
        </ul>
    ...
    <li>
        ...
        <ul>
            <li>gleichmäßig/individuell: sämtliche Spalten habe die gleiche Breite bzw. können durch die Formatierungsangaben in der Titelzeile festgelegt
                werden (s.o.)
            </li>
            <li>automatisch: die Breite jeder einzelnen Spalte wird abhängig vom Inhalt dynamisch angepaßt (Formatierungsangaben in der Titelzeile werden
                ignoriert)
            </li>
        </ul>
    </li>
    ...
    <li>
        ...
        <ul>
            <li>die Trennlinien werden stets in der
                <link>
                Zusatzvordergrundfarbe 1***1003</link> angezeigt
            </li>
        </ul>
        ...
        <ul>
            ...
            <li>
                ...
                <ul>
                    ...
                    <li>Inhalt (gesamt): KO2 wird auf den gesamten Inhalt der Zeile bzw. Zelle gesetzt (Zelleninhalte werden ggf. mit dem Spaltentrenner (s.o.)
                        zusammengefasst)
                    </li>
                    <li>Inhalt (ohne Spalte 1): KO2 wird auf den gesamten Inhalt der Zeile bzw. Zelle mit Ausnahme von der 1. Spalte gesetzt (Zelleninhalte
                        werden ggf. mit dem Spaltentrenner (s.o.) zusammengefasst)
                    </li>
