...
VSE_VSEID=function(elementId,obj,meta,property,isPreview,koValue) {
	...
	var n="<table cellpadding='0' cellspacing='0' border='0' style='position:absolute; left:0; top:0; width:100%; height:100%;'>";
		n+="<tr style='height:"+mheight+"px;'>";
			...
			n+="<td width='20%' align='center'>&gt;</td>";
		n+="</tr>";
		n+="<tr><td colspan='3' align='center' style='border-top:1px dotted;'>"+((isPreview)?"":"<span class='app2_pseudoElement'>ANRUFARCHIV</span>")+"</td></tr>";
	n+="</table>";
	obj.innerHTML=n;
...
<?
		if ($json1['callMode']==0) {$ss1=sql_call("SELECT * FROM edomiLive.archivPhoneData WHERE (targetid=".$json1['archivId']." AND typ=0) ORDER BY datetime DESC,ms DESC LIMIT 0,".$json1['cursor']);}
		if ($json1['callMode']==1) {$ss1=sql_call("SELECT * FROM edomiLive.archivPhoneData WHERE (targetid=".$json1['archivId']." AND typ=1) ORDER BY datetime DESC,ms DESC LIMIT 0,".$json1['cursor']);}
		if ($json1['callMode']==2) {$ss1=sql_call("SELECT * FROM edomiLive.archivPhoneData WHERE (targetid=".$json1['archivId'].") ORDER BY datetime DESC,ms DESC LIMIT 0,".$json1['cursor']);}
		$count=0;
...
?>
				n+="<td align='left' width='1'><?echo $typ;?></td>";
				n+="<td align='left' style='word-break:break-all;'><?echo escapeString($info,1);?></td>";
<?
				if ($json1['rowMode']==1) {
?>
					n+="<tr><td colspan='3'><div style='width:100%; height:1px;'></div></td></tr>";
<?
				} else if ($json1['rowMode']==2) {
?>
					n+="<tr><td colspan='3'><div style='width:100%; height:1px; border-bottom:1px solid; opacity:0.25;'></div></td></tr>";
<?
				}
?>
			n+="</tr>";
...
?>
			n+="<tr><td id='e-<?echo $json1['elementId'];?>-loadmore' colspan='3' align='center'><div style='margin:5px; padding:5px;'>&middot;&middot;&middot;</div></td></tr>";
			n+="<tr><td colspan='3'><div style='width:100%; height:1px; border-bottom:1px solid; opacity:0.25;'></div></td></tr>";
<?
		}
?>
...
###[HELP]###
Das Visuelement "Anrufarchiv" stellt den Inhalt eines <link>konfigurierten Anrufarchivs***1000-127</link> in der Visualisierung tabellarisch dar.
...
<ul>
	<li>Anrufarchiv: Auswahl des <link>konfigurierten Anrufarchivs***1000-127</link>, dessen Inhalt angezeigt werden soll</li>
...
<ul>
	<li>
		Titelleiste:
		<ul>
			<li>hier werden Pfeil-Schaltflächen zum Blättern durch die Einträge (Scrollen), sowie der Name des <link>Anrufarchivs***1000-127</link> angezeigt</li>
	...
	<li>
		...
		<ul>
			...
			<li>sofern ein zur Rufnummer passender Eintrag im <link>Telefonbuch***1000-125</link> vorliegt, wird der Name des Anrufers bzw. des Angerufenen angezeigt (ansonsten wird die Rufnummer angezeigt)</li>
