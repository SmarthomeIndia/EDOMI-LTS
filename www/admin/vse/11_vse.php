...
<ul>
	<li>
		...
		<ul>
			...
			<li>Inkrementalgeber (stets relativ): Der Drehregler verhält sich wie ein Inkrementalgeber, d.h. bei jeder Winkeländerung von 5 bzw. 15 Grad wird der Wert je nach Drehrichtung erhöht oder erniedrigt. Die Schrittweite der Wertänderung ist dabei von den u.g. Parametern abhängig (Raster und Nachkommastellen).</li>
			<li>relativ: Der Wert von KO2 wird relativ zum aktuellen Wert (KO1) verändert, d.h. der Drehregler kann an einer beliebigen Position "angefasst" werden, ohne dass eine Wertänderung erfolgt. Erst beim Bewegen des Drehreglers wird der Wert relativ zu dieser Startposition abgeändert.</li>
			<li>absolut: Der Wert von KO2 wird unabhängig vom aktuellen Wert (KO1) gesetzt, d.h. beim "Anfassen" des Drehreglers wird bereits der mit dieser Position korrespondierende Wert gesetzt.</li>
	...
	<li>
		...
		<ul>
			...
			<li>wird dieses Feld [leer] belassen, werden ggf. die KO-Filtereinstellungen angewendet</li>
			<li>Hinweis: Im Modus "Inkrementalgeber" legt dieser Wert ggf. die Schrittweite jeder Wertänderung fest. Wird kein Rasterwert angegeben, ist die Anzahl der Nachkommastellen (s.u.) ausschlaggebend: Der Wert wird z.B. bei 2 Nachkommastellen um 0.01 erhöht bzw. erniedrigt.
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>Hinweis: Im Modus "Inkrementalgeber" legt dieser Wert ggf. die Schrittweite jeder Wertänderung fest, sofern kein Rasterwert (s.o.) angegeben wurde: Der Wert wird z.B. bei 2 Nachkommastellen um 0.01 erhöht bzw. erniedrigt.
		</ul>
	</li>
	...
	<li>
		Verbundenes Visuelement (Knopf): falls ein Visuelement mit dem Drehregler <link>verbunden***2</link> wurde, wird das verbundene Visuelement an der aktuellen Knopf-Position angezeigt (ggf. auch rotiert)
		<ul>
			<li>Radius: relativer Abstand des verbundenen Visuelements vom Mittelpunkt der grafischen Anzeige des Drehreglers (siehe "Größe"), erlaubt sind Angaben im Bereich -&infin;..&infin;</li>
...
<ul>
	...
	<li>ggf. kann hier die Option "Live-Vorschau" aktiviert werden, um das Universalelement in Echtzeit während der Bedienung des Drehreglers reagieren zu lassen</li>
	<li>im Design des Visuelements wird der Eigenschaft "X-Position" die Formel "{polarX(...)}" und der Eigenschaft "Y-Position" die Formel "{polarY(...)}" zugewiesen (die Funktionsparameter werden in der Folge erläutert)</li>
	...
	<li>falls das Seitenverhältnis des Drehreglers nicht 1:1 beträgt, muss die Position des Universalelements entsprechend angepaßt werden: Ziel ist es das Universalelement in der linken oberen Ecke einer gedachten rechteckigen Begrenzung um die Kreisbahn des Drehreglers zu positionieren.</li>
</ul>

Die Parameter der o.g. Formeln <link>polarX(minValue,maxValue,minAngle,maxAngle,Radius)***r-3</link> bzw. <link>polarY(...)***r-3</link> sind wie folgt zu wählen:
<ul>
	<li>minValue/maxValue: der Wertebereich (Minimum/Maximum) des Drehreglers</li>
	<li>minAngle/maxAngle: der Winkelbereich des Drehreglers (beim Modus "360 Grad" bzw. "Inkrementalgeber" sind 0/360 anzugeben, beim Modus "270 Grad" sind 45/315 anzugeben)</li>
	<li>Radius: der Radius entspricht der halben Breite des Drehreglers (ggf. abzüglich Rahmenbreite), bzw. dessen gedachter rechteckiger Begrenzung seiner Kreisbahn</li>
	<li>Hinweis: Der Radius kann ggf. angepaßt werden, z.B. um das Universalelement (Knopf) exakt auf der Kreisbahn zu positionieren. In der Regel ist es z.B. sinnvoll, die halbe "Größe" des Knopfes vom Radius abzuziehen, damit der Knopf mit seinem Mittelpunkt auf der Kreisbahn liegt.</li>
</ul>

<b>Hinweis:</b> 
Zur Positionsanzeige kann alternativ (oder zusätzlich) z.B. auch eine <link>Wertanzeige***1002-27</link> verwendet werden.
