...
VSE_VSEID_CONSTRUCT=function(elementId,obj) {
	var n="<div id='e-"+elementId+"-svgcontainer' style='display:block; position:absolute; left:0; top:0; width:100%; height:100%;'>";
		n+="<svg id='e-"+elementId+"-svg' style='display:block; left:0; top:0; width:100%; height:100%; box-sizing:border-box;'></svg>";
	n+="</div>";
	n+="<div style='display:block; position:absolute; left:0; top:0; right:0; bottom:0; pointer-events:none;'>";
		n+="<table cellpadding='0' cellspacing='0' border='0' style='width:100%; height:100%;'><tr><td><span id='e-"+elementId+"-text'></span></td></tr></table>";
	n+="</div>";
...
VSE_VSEID_render=function(elementId,isActive,koValue) {
	...
	function track(fgcolor,pos,para) {
		if (size.o==0) {
			if (obj.dataset.var13>=2) {
				...
				if (x2>=x1) {r+="<line x1='"+x1+"' y1='"+(svgHeight/2)+"' x2='"+x2+"' y2='"+(svgHeight/2)+"' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";}
				var x1=(svgWidth/2+svgWidth/2*para.size/100);
				var x2=(pos+(svgHeight/2*para.knobsize/100));
				if (x2<=x1) {r+="<line x1='"+x1+"' y1='"+(svgHeight/2)+"' x2='"+x2+"' y2='"+(svgHeight/2)+"' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";}
				return r;
			} else {
				return "<line x1='"+(svgWidth/2-svgWidth/2*para.size/100)+"' y1='"+(svgHeight/2)+"' x2='"+(svgWidth/2+svgWidth/2*para.size/100)+"' y2='"+(svgHeight/2)+"' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";
			}
		} else {
			if (obj.dataset.var13>=2) {
				var r="";
				...
				if (y2>=y1) {r+="<line y1='"+y1+"' x1='"+(svgWidth/2)+"' y2='"+y2+"' x2='"+(svgWidth/2)+"' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";}
				var y1=(svgHeight/2+svgHeight/2*para.size/100);
				var y2=(pos+(svgWidth/2*para.knobsize/100));
				if (y2<=y1) {r+="<line y1='"+y1+"' x1='"+(svgWidth/2)+"' y2='"+y2+"' x2='"+(svgWidth/2)+"' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";}
				return r;
			} else {
				return "<line x1='"+(svgWidth/2)+"' y1='"+(svgHeight/2-svgHeight/2*para.size/100)+"' x2='"+(svgWidth/2)+"' y2='"+(svgHeight/2+svgHeight/2*para.size/100)+"' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";
			}
		}
	}
	...
	function knob(fgcolor,pos,para) {
		if (size.o==0) {
			if (obj.dataset.var13==1) {return "<line x1='"+pos+"' y1='"+(svgHeight/2-svgHeight/2*para.size/100)+"' x2='"+pos+"' y2='"+(svgHeight/2+svgHeight/2*para.size/100)+"' stroke-linecap='butt' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";}
			if (obj.dataset.var13==2) {return "<circle cx='"+pos+"' cy='"+(svgHeight/2)+"' r='"+(svgHeight/2*para.size/100)+"' stroke-linecap='butt' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='"+visuElement_getBgColor(obj,1)+"'/>";}
			if (obj.dataset.var13==3) {return "<rect x='"+(pos-svgHeight/2*para.size/100)+"' y='"+(svgHeight/2-svgHeight/2*para.size/100)+"' width='"+(svgHeight*para.size/100)+"' height='"+(svgHeight*para.size/100)+"' stroke-linecap='butt' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='"+visuElement_getBgColor(obj,1)+"'/>";}
		} else {
			if (obj.dataset.var13==1) {return "<line x1='"+(svgWidth/2-svgWidth/2*para.size/100)+"' y1='"+pos+"' x2='"+(svgWidth/2+svgWidth/2*para.size/100)+"' y2='"+pos+"' stroke-linecap='butt' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='none'/>";}
			if (obj.dataset.var13==2) {return "<circle cx='"+(svgWidth/2)+"' cy='"+pos+"' r='"+(svgWidth/2*para.size/100)+"' stroke-linecap='butt' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='"+visuElement_getBgColor(obj,1)+"'/>";}
			if (obj.dataset.var13==3) {return "<rect x='"+(svgWidth/2-svgWidth/2*para.size/100)+"' y='"+(pos-svgWidth/2*para.size/100)+"' width='"+(svgWidth*para.size/100)+"' height='"+(svgWidth*para.size/100)+"' stroke-linecap='butt' stroke='"+fgcolor+"' stroke-width='"+para.width+"' vector-effect='non-scaling-stroke' fill='"+visuElement_getBgColor(obj,1)+"'/>";}
		}
	}
}
...
<ul>
	<li>
		...
		<ul>
			<li>Potentiometer (relativ): Der Wert von KO2 wird relativ zum aktuellen Wert (KO1) verändert, d.h. der Schieberegler kann an einer beliebigen Position "angefasst" werden, ohne dass eine Wertänderung erfolgt. Erst beim Bewegen des Schiebereglers wird der Wert relativ zu dieser Startposition abgeändert.</li>
			<li>Potentiometer (absolut): Der Wert von KO2 wird unabhängig vom aktuellen Wert (KO1) gesetzt, d.h. beim "Anfassen" des Schiebereglers wird bereits der mit dieser Position korrespondierende Wert gesetzt.</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			<li>Hinweis: die Ausrichtung des Schiebereglers (horizontal oder vertikal) erfolgt stets automatisch anhand des Seitenverhältnisses (Breite:Höhe): Ist "Breite &ge; Höhe" wird der Schieberegler horizontal ausgerichtet, ansonsten vertikal.</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>Kreis: der Knopf wird als Kreiskontur angezeigt, optional wird der Kreis mit der <link>Zusatzhintergrundfarbe 1***1003</link> gefüllt</li>
			<li>Quadrat: der Knopf wird als Quadratkontur angezeigt, optional wird der Kreis mit der <link>Zusatzhintergrundfarbe 1***1003</link> gefüllt</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>Wichtig: die Farbe der Schleifspur wird mit der <link>Zusatzvordergrundfarbe 1***1003</link> festgelegt</li>
		</ul>
	</li>
	...
	<li>
		Verbundenes Visuelement (Knopf): falls ein Visuelement mit dem Schieberegler <link>verbunden***2</link> wurde, wird das verbundene Visuelement an der aktuellen Knopf-Position angezeigt (ggf. auch rotiert)
		<ul>
			<li>Abstand von Mittellinie: relativer Abstand des verbundenen Visuelements von der Mittellinie der grafischen Anzeige des Schiebereglers (siehe "Größe"), erlaubt sind Angaben im Bereich -&infin;..&infin;</li>
...
<ul>
	...
	<li>ggf. kann hier die Option "Live-Vorschau" aktiviert werden, um das Universalelement in Echtzeit während der Bedienung des Schiebereglers reagieren zu lassen</li>
	<li>je nach Ausrichtung des Schiebereglers (horizontal oder vertikal) wird im Design des Visuelements der Eigenschaft "X-Position" (horizontal) oder "Y-Position" (vertikal) die Formel "{range(...)}" zugewiesen (die Funktionsparameter werden in der Folge erläutert)</li>
	...
	<li>Hinweis: Die genaue Position kann ggf. angepaßt werden, z.B. um das Universalelement (Knopf) exakt auf dem Pfad zu positionieren. In der Regel ist es z.B. sinnvoll, den Knopf mittig auf der entsprechenden Kante zu positionieren.</li>
</ul>

Die Parameter der o.g. Formel <link>range(minValue,maxValue,Range)***r-3</link> sind wie folgt zu wählen:
<ul>
...
<b>Hinweis:</b> 
Zur Positionsanzeige kann alternativ (oder zusätzlich) z.B. auch eine <link>Wertanzeige***1002-27</link> oder eine <link>dynamische Hintergrundfarbe***1000-25</link> verwendet werden.
