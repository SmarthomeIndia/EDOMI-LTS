...
function cmd($cmd)
{
    ...
    if ($cmd == 'initApp') { ?>
        ...
        <div class='appWindowDrag' id='<? echo $winId; ?>-global'>";
            n+="
            <div class='appTitelDrag' onMouseDown='dragWindowStart(\"<? echo $appId; ?>\",\"<? echo $winId; ?>-global\");'>Befehle (Ausgangsbox)
                <div class='cmdClose cmdCloseDisabled'></div>
                <div class='cmdHelp' onClick='openWindow(9999,\"<? echo $appId; ?>\");'></div>
            </div>
            ";
            n+="
            <div id='<? echo $winId; ?>-main' style='width:500px;'></div>
            ";
            n+="
        </div>";
    ...
    if ($cmd == 'start') { ?>
        ...
        <div class='appMenu'>";
            n+="
            <div class='cmdButton' onClick='controlReturn(\"<? echo $winId; ?>\",\"<? echo $dataArr[0]; ?>\",\"<? echo $dataArr[2]; ?>\");'><b>Übernehmen</b>
            </div>
            ";
            n+="
        </div>";
        n+="
        <div class='appContentBlank' id='<? echo $winId; ?>-form1'>";
            n+="
            <div id='<? echo $winId; ?>-list1' data-type='1007' data-value='<? echo $dataArr[2]; ?>' data-itemid='0' data-db='<? echo $dataArr[5]; ?>'
                 class='controlList' style='height:400px; border:none;'>&nbsp;
            </div>
            ";
            n+="
        </div>";
    ...
    if ($cmd == 'validateValue') {
        if ($phpdataArr[0] != '') {
            echo 'controlReturn("' . $winId . '","' . $dataArr[0] . '","' . $phpdataArr[0] . '");';
        } else {
            echo 'shakeObj("' . $winId . '");';
        }
    }
