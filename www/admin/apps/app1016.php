...
function cmd($cmd)
{
    ...
    if ($cmd == 'initApp') {
        ...
        $fd[1] = $phpdataArr[1];
        $fd[2] = $phpdataArr[2];
        $fd[3] = file_get_contents(MAIN_PATH . '/www/admin/vse/' . $phpdataArr[1] . '_vse.php'); ?>
        var n="
        <div class='appWindowFullscreen'>";
            n+="
            <div class='appTitel'>Visuelement <span class='idBig'><? echo $fd[1]; ?></span>
                ...
                <div class='appMenu'>";
                    <? if (dbRoot_getRootId($fd[2]) == 170) { ?>
                        ...
                        <div class='cmdButton cmdButtonR'
                             onClick='ajax(\"checkVse\",\"<? echo $appId; ?>\",\"<? echo $winId; ?>\",\"<? echo $data; ?>\",controlGetFormData(\"<? echo $winId; ?>-form1\"));'>
                            <b>Übernehmen</b></div>";
                    <? } else { ?>
                        n+="
    ...
    <? }
    if ($cmd == 'checkVse') {
        $f = fopen(MAIN_PATH . '/www/admin/vse/' . $phpdataArr[1] . '_vse.php', 'w');
