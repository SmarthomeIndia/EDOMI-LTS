...
sql_connect();
if (checkAdmin($sid, false)) {
    $fn = trim(httpGetVar('filename'));
    if (basename($fn) == $fn) {
        if (file_exists(MAIN_PATH . '/www/data/tmp/' . $fn)) {
            header('Location: ../../data/tmp/' . basename($fn));
        }
    } else {
        if (file_exists($fn)) {
            exec('tar -cf "' . MAIN_PATH . '/www/data/tmp/' . basename($fn) . '.tar" -C "' . dirname($fn) . '" "' . basename($fn) . '"');
            header('Location: ../../data/tmp/' . basename($fn) . '.tar');
        }
    }
} else { ?>
