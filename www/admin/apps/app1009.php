...
?><? ?><? require("../../shared/php/config.php"); require(MAIN_PATH."/www/shared/php/base.php"); require(MAIN_PATH."/www/shared/php/incl_http.php"); require(MAIN_PATH."/www/admin/include/php/config.php"); require(MAIN_PATH."/www/admin/include/php/base.php"); require(MAIN_PATH."/www/admin/include/php/incl_items.php"); sql_connect(); if (checkAdmin($sid)) {cmd($cmd);} function cmd($cmd) { global $appId,$winId,$data,$dataArr,$phpdata,$phpdataArr,$sid; if ($cmd=='initApp') { ?>
		var n="<div class='appWindowDrag' id='<?echo $winId;?>-global'>";
			n+="<div class='appTitelDrag' onMouseDown='dragWindowStart(\"<?echo $appId;?>\",\"<?echo $winId;?>-global\");'>Kommunikationsobjekt (Anwesenheitssimulation)<div class='cmdClose' onClick='closeWindow(\"<?echo $winId;?>\");'></div><div class='cmdHelp' onClick='openWindow(9999,\"<?echo $appId;?>\");'></div></div>";
			n+="<div id='<?echo $winId;?>-main' style='width:550px;'></div>";
		n+="</div>";
		document.getElementById("<?echo $winId;?>").innerHTML=n;
		dragWindowRestore("<?echo $appId;?>","<?echo $winId;?>-global");
<? cmd('start'); } if ($cmd=='start') { $ss1=sql_call("SELECT * FROM edomiProject.editAwsList WHERE (id=".$dataArr[4].") ORDER BY id ASC"); if ($n=sql_result($ss1)) { $fd[1]=$n['id']; $fd[2]=$n['targetid']; $fd[3]=$n['gaid']; $fd[4]=$n['gavalue1']; $fd[5]=$n['gavalue2']; $fd[6]=$n['gaid2']; } else { $fd[1]=-1; $fd[2]=$dataArr[2]; $fd[3]=''; $fd[4]=''; $fd[5]=''; $fd[6]=''; } ?>
		...
		n+="<div id='<?echo $winId;?>-form1' class='appContent' style='padding:4px;'>";
			...
			n+="<table width='100%' border='0' cellpadding='5' cellspacing='0'>";
				n+="<tr><td colspan='2'>KO<br><div id='<?echo $winId;?>-fd3' data-type='1000' data-root='30' data-value='<?echo $fd[3];?>' data-options='typ=1;reset=0' class='control10' style='width:100%;'>&nbsp;</div></td></tr>";
				n+="<tr>";
					n+="<td>Startwert<br><input type='text' id='<?echo $winId;?>-fd4' data-type='1' value='' class='control1' style='width:100%;'></input></td>";
					n+="<td>Stoppwert<br><input type='text' id='<?echo $winId;?>-fd5' data-type='1' value='' class='control1' style='width:100%;'></input></td>";
				n+="</tr>";
				n+="<tr><td colspan='2'>Status-KO (optional)<br><div id='<?echo $winId;?>-fd6' data-type='1000' data-root='30' data-value='<?echo $fd[6];?>' data-options='typ=1;reset=1' class='control10' style='width:100%;'>&nbsp;</div></td></tr>";
			n+="</table>";
		n+="</div>";
...
<? } if ($cmd=='saveItem') { $dbId=db_itemSave('editAwsList',$phpdataArr); if ($dbId>0) { ?>
			controlReturn("<?echo $winId;?>","<?echo $dataArr[0];?>","<?echo $dataArr[2];?>");
<? } else { ?>
			shakeObj("<?echo $winId;?>");
<? } } if ($cmd=='deleteItem') { db_itemDelete('editAwsList',$phpdataArr[0]); ?>
