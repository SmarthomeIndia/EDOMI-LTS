...
function cmd($cmd)
{
    ...
    if ($cmd == 'initApp') { ?>
        ...
        <div class='appWindowDrag' id='<? echo $winId; ?>-global'>";
            n+="
            <div class='appTitelDrag' onMouseDown='dragWindowStart(\"<? echo $appId; ?>\",\"<? echo $winId; ?>-global\");'>Kommunikationsobjekt
            ...
            ";
            n+="
            <div id='<? echo $winId; ?>-main' style='width:550px;'></div>
            ";
            n+="
        </div>";
    ...
    if ($cmd == 'start') {
        $ss1 = sql_call("SELECT * FROM edomiProject.editAwsList WHERE (id=" . $dataArr[4] . ") ORDER BY id ASC");
        if ($n = sql_result($ss1)) {
            $fd[1] = $n['id'];
            $fd[2] = $n['targetid'];
            $fd[3] = $n['gaid'];
        ...
        <div id='<? echo $winId; ?>-form1' class='appContent' style='padding:4px;'>";
            ...
            <table width='100%' border='0' cellpadding='5' cellspacing='0'>";
                ...
                <tr>";
                    n+="
                    <td>Startwert<br><input type='text' id='<? echo $winId; ?>-fd4' data-type='1' value='' class='control1' style='width:100%;'></input></td>
                    ";
                    n+="
                    <td>Stoppwert<br><input type='text' id='<? echo $winId; ?>-fd5' data-type='1' value='' class='control1' style='width:100%;'></input></td>
                    ";
                    n+="
                </tr>
                ";
                n+="
                <tr>
                    <td colspan='2'>Status-KO (optional)<br>
                        <div id='<? echo $winId; ?>-fd6' data-type='1000' data-root='30' data-value='<? echo $fd[6]; ?>' data-options='typ=1;reset=1'
                             class='control10' style='width:100%;'>&nbsp;
                        </div>
                    </td>
                </tr>
    ...
    <? }
    if ($cmd == 'saveItem') {
        $dbId = db_itemSave('editAwsList', $phpdataArr);
        if ($dbId > 0) { ?>
    ...
    if ($cmd == 'deleteItem') {
        db_itemDelete('editAwsList', $phpdataArr[0]); ?>
