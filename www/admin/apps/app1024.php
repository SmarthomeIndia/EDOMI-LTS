...
function cmd($cmd)
{
    ...
    if ($cmd == 'initApp') { ?>
        ...
        <div class='appWindowDrag' id='<? echo $winId; ?>-global'>";
            n+="
            <div class='appTitelDrag' onMouseDown='dragWindowStart(\"<? echo $appId; ?>\",\"<? echo $winId; ?>-global\");'>Makro-Vorgabe (Terminschaltuhr)
    ...
    if ($cmd == 'start') {
        $ss1 = sql_call("SELECT * FROM edomiProject.editAgendaMacroList WHERE (id=" . $dataArr[4] . ") ORDER BY id ASC");
        if ($n = sql_result($ss1)) {
            $fd[1] = $n['id'];
            $fd[2] = $n['agendaid'];
            $fd[3] = $n['targetid'];
    ...
    <? }
    if ($cmd == 'saveItem') {
        $dbId = db_itemSave('editAgendaMacroList', $phpdataArr);
        if ($dbId > 0) { ?>
    ...
    if ($cmd == 'deleteItem') {
        db_itemDelete('editAgendaMacroList', $phpdataArr[0]); ?>
        controlReturn("","<? echo $dataArr[0]; ?>","<? echo $dataArr[2]; ?>");
    <? }
    if ($cmd == 'sortDecItem' || $cmd == 'sortIncItem') {
        $ss1 = sql_call("SELECT id,sort FROM edomiProject.editAgendaMacroList WHERE (id=" . $phpdataArr[0] . ")");
        if ($item = sql_result($ss1)) {
            if ($cmd == 'sortDecItem') {
                $ss2 = sql_call("SELECT id,sort FROM edomiProject.editAgendaMacroList WHERE (agendaid=" . $dataArr[2] . " AND sort<" . $item['sort'] . ") ORDER BY sort DESC LIMIT 0,1");
            }
            if ($cmd == 'sortIncItem') {
                $ss2 = sql_call("SELECT id,sort FROM edomiProject.editAgendaMacroList WHERE (agendaid=" . $dataArr[2] . " AND sort>" . $item['sort'] . ") ORDER BY sort ASC LIMIT 0,1");
            }
            if ($n = sql_result($ss2)) {
                sql_call("UPDATE edomiProject.editAgendaMacroList SET sort=" . $n['sort'] . " WHERE (id=" . $phpdataArr[0] . ")");
                sql_call("UPDATE edomiProject.editAgendaMacroList SET sort=" . $item['sort'] . " WHERE (id=" . $n['id'] . ")");
            }
        } ?>
        controlReturn("","<? echo $dataArr[0]; ?>","<? echo $dataArr[2]; ?>");
    <? }
