...
function LB_LBSID($id) {
	if (($E=logic_getInputs($id)) && ($V=logic_getVars($id))) {
		...
		if ($E[3]['refresh']==1 && $E[3]['value']==0 && ($V[1]==1 || $V[1]==2 || $V[1]==3 || $V[1]==11 || $V[1]==12 || $V[1]==13)) {
			if ($V[1]==1) {logic_setOutput($id,2,1);}
			else if ($V[1]==2) {logic_setOutput($id,2,2);}
			else if ($V[1]==3) {logic_setOutput($id,2,3);}
			else if ($V[1]==11) {logic_setOutput($id,4,1);}
			else if ($V[1]==12) {logic_setOutput($id,4,2);}
			else if ($V[1]==13) {logic_setOutput($id,4,3);}
