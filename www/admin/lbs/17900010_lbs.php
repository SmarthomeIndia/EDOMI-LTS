...
<ul>
	...
	<li>
		1: "Automatik"
		<ul>
			<li>dies ist die normale Betriebsart, d.h. je nach Wert an E2 wird über den Skimmer gefiltert und ggf. wird der Pegel angepasst oder eine Chlorierung durchgeführt</li>
	...
	<li>
		...
		<ul>
			...
			<li>Ventilstellung: "Skimmer&rarr;Filter&rarr;Jet" (erwartet wird das Anschließen des Saugers an den Skimmer, d.h. die Ventilstellung entspricht der beim Filtern)</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>beendet wird das Saugen i.d.R. durch einen Wechsel in die Betriebsart "Automatik" (E1=1) oder in eine beliebige andere Betriebsart</li>
			<li>Ventilstellung: "Sauger&rarr;(Filter)&rarr;Drain" (der Sandfilter wird trotz der Ventilstellung "Filter" umgangen, sofern die Installation entsprechend ausgelegt ist)</li>
		</ul>
	</li>
	...
	<li>
		...
		<ul>
			...
			<li>ein Wechsel in eine andere Betriebsart mittels E1 ist in dieser Betriebsart nicht möglich (mit Ausnahme von "Aus"), da E4 nunmehr die Betriebsart festlegt (s.u.)</li>
			<li>Ventilstellung: "Skimmer&rarr;Filter&rarr;Jet"</li>
		</ul>
	</li>
...
E3 kann z.B. mit Hilfe des <link>LBS 17900011***lbs_17900011</link> getriggert werden.
...
E4 kann z.B. mit Hilfe des <link>LBS 17900012***lbs_17900012</link> getriggert werden.


<h3>Wichtige Hinweise</h3>
<ul>
	<li>bei jedem Schaltvorgang von Umwälz- bzw. Chlorpumpe wird auf eine Statusrückmeldung an E9 bzw. E10 mit einem Timeout von 5 Sekunden gewartet (bleibt diese aus, wird in den Not-Aus-Modus gewechselt, s.u.)</li>
	<li>ebenso wird bei jedem Verfahren der Ventile auf eine Statusrückmeldung an E11..E14 mit einem Timeout (E5) gewartet (bleibt diese aus bzw. entspricht der Ventilstatus nicht den Erwartungen, wird in den Not-Aus-Modus gewechselt, s.u.)</li>
	<li>Die Schaltausgänge für die Pumpen und Ventile sind zur Sicherheit nicht in jeder Situation "SBC"-Ausgänge (nur bei Änderung), d.h. ggf. wird ein Ausgang mehrfach z.B. auf 0 gesetzt.</li>
	<li>Falls ein interner Fehler auftritt oder z.B. die erwartete Rückmeldung der Ventile ausbleibt, wird stets in einen Not-Aus-Modus gewechselt (d.h. die Ventile werden in einer sichere Stellung verfahren und alle Pumpen und das Zulaufventil werden abgeschaltet bzw. geschlossen). Der Baustein wird dann deaktiviert und muss über E1 erneut in die gewünschten Betriebsart versetzt werden.</li>
	<li>Wenn EDOMI beendet (bzw. neugestartet) wird, erfolgt ebenfalls ein Wechselt in den Not-Aus-Modus. Jedoch wird in diesem Fall nicht auf eine Rückmeldung der Ventile etc. gewartet.</li>
</ul>
...
*/
	...
	private function setWorkMode($mode) {
		$position=0;
		if ($mode==1) {$position=1; $pump=false;}
		if ($mode==2) {$position=1; $pump=true;}
		if ($mode==3) {$position=3; $pump=true;}
		if ($mode==4) {$position=1; $pump=true;}
		if ($mode==5) {$position=2; $pump=true;}
		if ($mode==6) {$position=2; $pump=true;}
		if ($mode==7) {$position=1; $pump=false;}
		if ($mode==8) {$position=1; $pump=true;}
	...
	private function getValves() {
		$tmp=0;
		if ($this->getE()) {
			if ($this->E[11]['value']==1 && $this->E[12]['value']==0 && $this->E[13]['value']==1 && $this->E[14]['value']==0) {$tmp=1;}	//Filtern/Standby/Saugen
			if ($this->E[11]['value']==0 && $this->E[12]['value']==1 && $this->E[13]['value']==1 && $this->E[14]['value']==0) {$tmp=2;}	//Saugen>Gulli/Abpumpen
			if ($this->E[11]['value']==0 && $this->E[12]['value']==1 && $this->E[13]['value']==0 && $this->E[14]['value']==1) {$tmp=3;}	//Backwash
		}
	...
	private function setPump($mode) {
		...
		$preState=0;
		if ($this->getE() && $this->E[10]['value']==1) {$preState=1;}
