...
function LB_LBSID($id) {
	if ($E=logic_getInputs($id)) {
		...
		if ($E[1]['refresh']==1 && $E[1]['value']>=1 && $E[1]['value']<=4) {
			...
			for ($t=1;$t<=4;$t++) {$E[$t]['value']=abs(intVal($E[$t]['value']));}
			
			if ($E[2]['value']==0) 		{$archivDb='archivKoData';}
			else if ($E[2]['value']==1) {$archivDb='archivMsgData';}
			else if ($E[2]['value']==2) {$archivDb='archivPhoneData';}
			else if ($E[2]['value']==3) {$archivDb='archivCamData';}		

			if ($E[3]['value']>0) {$archivId=$E[3]['value'];}
			...
			if (!isEmpty($archivDb) && $archivId>0 && $E[4]['value']>=0 && $E[4]['value']<=3) {
				...
				if ($E[4]['value']==0) 		{$ss1=sql_call("SELECT * FROM edomiLive.".$archivDb." WHERE (targetid=".$archivId.") ORDER BY datetime ASC,ms ASC LIMIT ".$E[5]['value'].",1");}
				else if ($E[4]['value']==1) {$ss1=sql_call("SELECT * FROM edomiLive.".$archivDb." WHERE (targetid=".$archivId.") ORDER BY datetime DESC,ms DESC LIMIT ".$E[5]['value'].",1");}
				else if ($E[4]['value']==2) {$ss1=sql_call("SELECT * FROM edomiLive.".$archivDb." WHERE (targetid=".$archivId.") ORDER BY ABS(CONCAT(UNIX_TIMESTAMP(datetime),LPAD(ms,6,'0'))-CONCAT(UNIX_TIMESTAMP('".$ts_Datetime."'),LPAD('".$ts_Ms."',6,'0'))) ASC LIMIT 0,1");}
				else if ($E[4]['value']==3) {$ss1=sql_call("SELECT * FROM edomiLive.".$archivDb." WHERE (targetid=".$archivId." AND datetime='".$ts_Datetime."' AND ms='".$ts_Ms."') LIMIT 0,1");}
				...
				if ($n=sql_result($ss1)) {
					...
					if ($E[2]['value']==0) 		{$sqlData=$n['gavalue'];}
					else if ($E[2]['value']==1) {$sqlData=$n['msg'];}
					else if ($E[2]['value']==2) {$sqlData=$n['phone'];}
					else if ($E[2]['value']==3) {$sqlData=getArchivCamFilename($n['targetid'],$n['camid'],$n['datetime'],$n['ms']).'.jpg';}
				
					if ($E[1]['value']==2) {					
						//Datensatz löschen
						if ($E[2]['value']==0) 		{executeCmd(array('cmd'=>50,'cmdid1'=>$archivId,'cmdoption1'=>3),array($n['datetime'],$n['ms']));}
						else if ($E[2]['value']==1) {executeCmd(array('cmd'=>51,'cmdid1'=>$archivId,'cmdoption1'=>3),array($n['datetime'],$n['ms']));}
						else if ($E[2]['value']==2) {executeCmd(array('cmd'=>53,'cmdid1'=>$archivId,'cmdoption1'=>3),array($n['datetime'],$n['ms']));}
						else if ($E[2]['value']==3) {executeCmd(array('cmd'=>52,'cmdid1'=>$archivId,'cmdoption1'=>3),array($n['datetime'],$n['ms']));}
						...
						if ($E[2]['value']==0) 		{sql_call("UPDATE edomiLive.".$archivDb." SET ".$sqlTS."gavalue='".sql_encodeValue($E[6]['value'])."' WHERE (targetid=".$archivId." AND datetime='".$n['datetime']."' AND ms='".$n['ms']."')");}
						else if ($E[2]['value']==1) {sql_call("UPDATE edomiLive.".$archivDb." SET ".$sqlTS."msg='".sql_encodeValue($E[6]['value'])."' WHERE (targetid=".$archivId." AND datetime='".$n['datetime']."' AND ms='".$n['ms']."')");}
						else if ($E[2]['value']==2) {sql_call("UPDATE edomiLive.".$archivDb." SET ".$sqlTS."phone='".sql_encodeValue($E[6]['value'])."' WHERE (targetid=".$archivId." AND datetime='".$n['datetime']."' AND ms='".$n['ms']."')");}
						else if ($E[2]['value']==3) {$error=true;}	//Kameraarchiv kann nicht überschrieben werden
					}
