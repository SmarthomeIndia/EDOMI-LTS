...
//### unsauber... besser hierhin kopieren bzw. Funktion (Intern ggf. aktivieren)
        ...
        if ($E[14]['value'] == 1 && $E[14]['refresh'] == 1) {
            if ($V[2] > 0) {
                if ($V[2] == 1) {
                    LB_LBSID_log($id, $E, 'Voralarm quittiert', 'Extern');
                }
                if ($V[2] == 2) {
                    LB_LBSID_log($id, $E, 'Alarm quittiert', 'Extern');
                }
                if ($V[2] == 4) {
                    LB_LBSID_log($id, $E, 'Voralarm quittiert', 'Intern');
                }
                if ($V[2] == 8) {
                    LB_LBSID_log($id, $E, 'Alarm quittiert', 'Intern');
                }
                if ($V[2] == 16) {
                    LB_LBSID_log($id, $E, 'Voralarm quittiert', 'Panik');
                }
                if ($V[2] == 32) {
                    LB_LBSID_log($id, $E, 'Alarm quittiert', 'Panik');
                }
