...
Ausschließlich E3 kann zudem auf den Wert -1 gesetzt werden: Dies entspricht einem Ausschalten, jedoch wird der Zustand
<i>nicht</i> erneut berechnet: Die Leuchte bleibt solange ausgeschaltet, bis ein neues Telegramm an E1..E10 oder E15 eintrifft (oder E16 von 0 auf 1 wechselt, sofern ein Sparmodus aktiviert ist). E2 hat jedoch auch in diesem Fall Priorität!

<b>Hinweis:</b>
Dieser Baustein kann das Status-KO einer Leuchte
<i>nicht</i> auswerten. Wenn die Leuchte zusätzlich über einen anderen Weg (ohne diesen Baustein) geschaltet wird, ist dem Baustein der aktuelle Zustand der Leuchte nicht bekannt - daher werden die Ausgänge A1..A6 u.U. nicht den tatsächlichen Zustand der Leuchte repräsentieren.
...
Bei Einbruch der Dunkelheit (E16=1) werden E2/E4/E5 erneut berechnet und die Leuchte ggf. eingeschaltet. Der Zustand von E3 wird dabei hingegen ignoriert, erst ein
<i>neues</i> Telegramm an E3 wird ggf. zu einer Statusänderung der Leuchte führen.
Beispiel: Wurde die Leuchte mittels eines Tastsensors an E3 eingeschaltet (z.B. bei Dunkelheit), wird die Leuchte bei eintretender Helligkeit automatisch ausgeschaltet. Bei erneutem Einbruch der Dunkelheit (z.B. am nächsten Abend) wird E3 u.U. noch immer z.B. auf den Wert 1 gesetzt sein und die Leuchte würde somit eingeschaltet werden (siehe bei E15=1). Mit E15=2 wird genau das verhindert, da erst ein
<i>neues</i> Telegramm an E3 bei Dunkelheit ausgewertet wird.
...
<ul>
    ...
    <li>E15: Sparmodus
        <ul>
            ...
            <li>1: Bei Einbruch der Dunkelheit (E16=1) werden E2..E5 erneut berechnet und die Leuchte ggf. eingeschaltet. Bei Helligkeit (E16=0) wird die
                Leuchte ausgeschaltet und E2..E5 werden ignoriert. Bei erneutem Einbruch der Dunkelheit wird wieder der aktuelle Zustand E2..E5 ermittelt und
                die Leuchte ggf. eingeschaltet.
            </li>
            <li>2: Wie zuvor, jedoch wird bei Einbruch der Dunkelheit E3 (Manuell) ignoriert. Erst ein <i>neues</i> Telegramm an E3 wird ggf. zu einer
                Statusänderung der Leuchte führen.
            </li>
...
<ul>
    ...
    <li>A3..A6: Status A..D: diese Ausgänge werden in Abhängigkeit von A2 auf 0 oder 1 gesetzt (0=AUS, 1=EIN)
        <ul>
            ...
            <li>daher kann stets maximal einer der Ausgänge A3..A6 den Wert 1 annehmen, da bedingt durch die Priorisierung stets nur ein Kanal A..D für den
                aktuellen Status an A2 verantwortlich sein kann
            </li>
            <li>Hinweis: Die einzelnen Ausgänge A3..A6 werden nur bei einer Änderung des jeweiligen Zustands gesetzt.</li>
            <li>Wichtig: Der Wert 0 an einem der Ausgänge A3..A6 bedeutet nicht zwangsläufig, dass ein AUS-Lichtwert an A1 anliegt - eine 0 bedeutet lediglich,
                dass der entsprechende Kanal nicht für einen EIN-Lichtwert verantwortlich ist. Nur wenn <i>alle</i> Ausgänge A3..A6 den Wert 0 haben, liegt an
                A1 ein AUS-Lichtwert an (bzw. A2 ist 0).
            </li>
        </ul>
    </li>
</ul>
...
function LB_LBSID($id)
{
    ...
    if (($E = logic_getInputs($id)) && ($V = logic_getVars($id))) {
        ...
        if (!($E[17]['value'] == 0 || $E[17]['value'] == 1 || $E[17]['value'] == 2 || $E[17]['value'] == 3)) {
            $E[17]['value'] = 0;
        }
        ...
        if ($tmp != $V[3]) {
            if ($V[3] < 0 || (($tmp & 2) ? 1 : 0) != (($V[3] & 2) ? 1 : 0)) {
                logic_setOutput($id, 3, (($tmp & 2) ? 1 : 0));
            }
            if ($V[3] < 0 || (($tmp & 4) ? 1 : 0) != (($V[3] & 4) ? 1 : 0)) {
                logic_setOutput($id, 4, (($tmp & 4) ? 1 : 0));
            }
            if ($V[3] < 0 || (($tmp & 8) ? 1 : 0) != (($V[3] & 8) ? 1 : 0)) {
                logic_setOutput($id, 5, (($tmp & 8) ? 1 : 0));
            }
            if ($V[3] < 0 || (($tmp & 16) ? 1 : 0) != (($V[3] & 16) ? 1 : 0)) {
                logic_setOutput($id, 6, (($tmp & 16) ? 1 : 0));
            }
