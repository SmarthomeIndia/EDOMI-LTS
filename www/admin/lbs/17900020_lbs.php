...
<ul>
	...
	<li>KO an E2 dient als Datenspeicher für diesen Baustein mit folgendem Aufbau: "CODE|DEVICE|FUNCTION|NAME;CODE|DEVICE|FUNCTION|NAME;..."
		<ul>
			...
			<li>DEVICE: 1/2/3 = der Datensatz ist beim Einlesen von einem der Lesegeräte 1..3 gültig, 0 = der Datensatz ist für alle Lesegeräte ungültig (funktionslos)</li>
...
<ul>
	...
	<li>beim Absetzes eines Befehls (E8) werden die aktuellen Werte an E3..E7 als Parameter übergeben (E3..E7 können z.B. in einer Visualisierung mit Eingabefeldern editierbar präsentiert werden)</li>
	...
	<li>folgende Wertzuweisungen an E8 lösen den entsprechenden Befehl aus:
		<ul>
			<li>1..&infin;: Datensatz mit der ID 1..&infin; abrufen (nur im Programmiermodus)
				<ul>
					<li>ein vorhandener Datensatz (mit der ID 1..&infin;) wird abgerufen, E3..E7 werden mit den Werten des soeben abgerufenen Datensatzes aktualisiert</li>
				</ul>
			</li>
			...
			<li>-1: Programmiermodus aktivieren (nur im normalen Betrieb)
				<ul>
					<li>bei aktiviertem Programmiermodus führt ein Einlesevorgang lediglich zur Ausgabe des eingelesenen Codes an E4, zudem werden alle entsprechenden Datensätze (mit diesem Code) hervorgehoben (siehe A9)</li>
					<li>der Programmiermodus wird ggf. automatisch nach Ablauf der festgelegten Zeit (E16) beendet</li>
				</ul>
			</li>
			...
			<li>-3: Hinzufügen eines neuen Datensatzes (nur im Programmiermodus)
				<ul>
					...
					<li>nach einem erfolgreichen Hinzufügen eines Datensatzes werden E3..E7 mit den Werten des soeben hinzugefügten Datensatzes aktualisiert</li>
				</ul>
			</li>

			<li>-4: Abrufen eines vorhandenen Datensatzes (nur im Programmiermodus)
				<ul>
					<li>E3=1..&infin;: ein vorhandener Datensatz (mit der ID 1..&infin; an E3) wird abgerufen, E3..E7 werden mit den Werten des soeben abgerufenen Datensatzes aktualisiert</li>
				</ul>
			</li>

			<li>-5: Filtern von vorhandenen Datensätzen (nur im Programmiermodus)
				<ul>
					<li>filtert die Ausgabe an A9 in Abhängigkeit von den Werten an E4..E7 (UND-verknüpft), alle den Filterkriterien entsprechenden Datensätze werden hervorgehoben (siehe A9)</li>
					<li>ein Wert=[leer] an E4..E7 führt dazu, dass der entsprechende Parameter ignoriert wird</li>
					<li>der Wert an E7 (Name) bestimmt auch die Anzahl der zu suchenden Zeichen, d.h. E7="Abc" findet auch Name="Abcde", nicht jedoch Name="Ab" (Groß- und Kleinschreibung wird ignoriert)</li>
				</ul>
			</li>
...
<ul>
	<li>
		...
		<ul>
			<li>Hinweis: "ungültiger Eintrag" bedeutet, dass der eingelesene Code zwar als Eintrag vorhanden ist, jedoch für das einlesenede Lesegerät nicht freigegeben oder keine gültige Funktion 1..&infin; zugewiesen wurde</li>
		</ul>
	</li>
</ul>
...
<ul>
	<li>ist das automatische Beenden des Programmiermodus aktiviert, wird der interne Timer bei jedem Befehl und bei jedem Einlesen eines Codes zurückgesetzt (ähnlich wie bei einem Bildschirmschoner)</li>
</ul>
...
<ul>
	...
	<li>Hinweis: Im Regelfall sollte A1 mit einem Vergleicher verbunden werden, um je nach Funktions-ID die gewünschte Funktion (z.B. Türöffnung) auslösen zu können.</li>
</ul>
...
<ul>
	...
	<li>Hinweis: A9 wird immer dann aktualisiert, wenn E2 getriggert wird oder ein entsprechender Befehl (E8) abgesetzt wird (bei jeder Änderung der Datensätze wird A9 aktualisiert)</li>
</ul>
...
function LB_LBSID($id) {
	if ($E=logic_getInputs($id)) {
		if (logic_getStateExec($id)==0) {
			if ($E[1]['refresh']==1 && $E[1]['value']!=0 && (!isEmpty($E[17]['value']))) {logic_callExec(LBSID,$id);}
...
class LB_LBSID_wiegand {
		...
		if ($this->E=logic_getInputs($this->id)) {
			...
			do {
				if ($this->tty_open()) {
					$this->proc_loop();
					$this->tty_close();
					if (!$this->exitLBS) {sleep(1);}	//Reconnect-Pause
				}
	...
	private function tty_open() {
		do {
			if ($this->E=logic_getInputs($this->id)) {
				if ($this->E[1]['value']==0) {$this->exitLBS=true;}
				if (!$this->exitLBS) {
					$err=0;
					$tmp=array();
	...
	private function setBlockTimer() {
		if ($this->blockMaxTimeout>0) {
			...
			if ($this->block_timeout>$this->blockMaxTimeout) {$this->block_timeout=$this->blockMaxTimeout;}
	...
	private function checkPrgTimeout() {
		if ($this->prgModeTimeout>0 && $this->prgMode && (getMicrotime()-$this->prg_timer)>=$this->prgModeTimeout) {return true;}
		return false;
	}
	...
	private function checkCmdTimer() {
		if ($this->cmd_timer===false || (getMicrotime()-$this->cmd_timer)>=$this->cmd_interval) {return true;}
		return false;
	}
	...
	private function parseResponse($rx) {
		...
		if ($par[0]=='R') {
			if ($this->prgMode) {
				...
				if ($par[2]==1) {$this->out(5,$par[1]);}
				if ($par[2]==2) {$this->out(6,$par[1]);}
				if ($par[2]==3) {$this->out(7,$par[1]);}
			}
		}
...
- die Ausgaben beginnen immer mit "#" und enden mit CHR(10)
  ...
  #R;<CODE>;<READDEVICE>		<CODE> wurde von <READDEVICE> eingelesen
  #E;<READDEVICE>				Lesefehler an <READDEVICE> (kein 26-Bit-Code oder sonstiges)
*/

#define PING_INTERVAL 10            //HEARTBEAT-Intervall (s)

#define PIN_INVERT true             //true=Eingänge für Wiegand-Data invertieren und Pullup aktivieren (z.B. bei Verwendung von Optokopplern mit invertiertem Eingang), false=Eingänge nicht invertieren
...
long getIdFromReaders() {
  ...
  do {
    ...
    if (((reader==0 && PIN_READER1_D0>0) || (reader==1 && PIN_READER2_D0>0) || (reader==2 && PIN_READER3_D0>0)) && (readerId<0 || readerId==reader)) {
      if (reader==0) {
