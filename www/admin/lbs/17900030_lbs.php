...
class LB_LBSID_remote {
	...
	private function proc_loop() {
		while (logic_getEdomiState()==1) {
			...
			if ($this->checkPollTimer()) {
				...
				if ($this->remotePower==1) {$this->out(1,1,true);} else {$this->out(1,0,true);}
				if ($this->remoteDisplay==1) {$this->out(2,1,true);} else {$this->out(2,0,true);}
	...
	private function checkPollTimer() {
		if ($this->poll_timer===false || (getMicrotime()-$this->poll_timer)>=$this->poll_interval) {return true;}
		return false;
	}
	...
	private function getRemoteStatus() {
		...
		if ($this->getPowerStatus()) {
			$this->remotePower=1;
			if ($this->getDisplayStatus()) {$this->remoteDisplay=1;}
		}
	}
	...
	private function getDisplayStatus() {
		...
		foreach ($tmp as $n) {
			$n=trim($n);
			$nn=explode(' ',$n,3);
			if (strToUpper($nn[0])=='MONITOR' && strToUpper($nn[2])=='ON') {return true;}
		}
		return false;
	}
